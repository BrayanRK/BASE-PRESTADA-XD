...
const isMp4 = (buffer: Buffer): boolean => {
  return buffer.length > 12 && buffer.subarray(4, 8).toString("ascii") === "ftyp"
}

const assertBuffer = (source: Buffer): void => {
...
const extractInputs = (html: string): Record<string, string> => {
...
const findConvertedMp4 = (html: string, baseUrl: string): string | null => {
  const source =
    html.match(/<source\b[^>]*src=["']([^"']+)["'][^>]*>/i)?.[1] ||
...
const fetchBuffer = async (url: string): Promise<Buffer> => {
  ...
  if (!buffer.length) throw new Error("Ezgif devolvió un MP4 vacío.")

  return buffer
}

const localWebp2Mp4 = async (source: Buffer): Promise<Buffer> => {
  try {
    const out = await ffmpegWithInputArgs(
      source,
      ["-ignore_loop", "0"],
      [
        "-t",
    ...
    if (isMp4(out.data)) return out.data
  } catch {}
...
const ezgifWebp2Mp4 = async (source: Buffer): Promise<Buffer> => {
  ...
  if (!inputs.file) form2.append("file", file)
...
export const webp2png = async (source: Buffer): Promise<Buffer> => {
  assertBuffer(source)
...
export const webp2mp4 = async (source: Buffer): Promise<Buffer> => {
  assertBuffer(source)

  const local = await localWebp2Mp4(source).catch(() => null)
  if (local?.length && isMp4(local)) return local
