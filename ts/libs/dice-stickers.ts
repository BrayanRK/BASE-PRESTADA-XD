...
export type DiceKind = "sticker" | "video" | "image"
...
const diceDir = (): string => {
  const dir = path.join(process.cwd(), "database", "dice")
  if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true })
  return dir
}

const manifestPath = (): string => path.join(diceDir(), "manifest.json")

const loadManifest = (): DiceEntry[] => {
  try {
...
const saveManifest = (entries: DiceEntry[]): void => {
  fs.writeFileSync(manifestPath(), JSON.stringify(entries, null, 2))
}

export const listDiceEntries = (): DiceEntry[] => {
  return loadManifest().sort((a, b) => a.slot - b.slot)
}

export const getDiceEntry = (slot: number): DiceEntry | null => {
  return loadManifest().find((entry) => entry.slot === slot) || null
}

export const nextEmptySlot = (): number | null => {
...
export const addDiceEntry = (
  ...
  const entry: DiceEntry = { slot, file, kind, mimetype, addedBy, addedAt: Date.now() }
  entries.push(entry)
...
export const readDiceBuffer = (entry: DiceEntry): Buffer | null => {
  try {
...
export const getRandomDiceEntry = (): DiceEntry | null => {
