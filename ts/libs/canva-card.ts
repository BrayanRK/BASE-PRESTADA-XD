...
export const fetchLyoQuoteCard = async (input: QuoteCardInput): Promise<Buffer> => {
  const body = {
    type: "quote",
    format: "png",
  ...
  if (response.status === 429) throw new Error("lyo.su sin límite de peticiones (rate-overlimit)")
  if (!response.ok) throw new Error(`lyo.su HTTP ${response.status}`)

  const json: any = await response.json().catch(() => null)
  const base64 = json?.result?.image
  if (typeof base64 !== "string" || !base64) throw new Error("lyo.su devolvió una respuesta inválida")

  const buffer = Buffer.from(base64, "base64")
  if (!buffer.length) throw new Error("lyo.su devolvió una imagen vacía")
  return buffer
}
...
const drawEllipsisText = (ctx: any, text: string, x: number, y: number, maxWidth: number): void => {
  let value = text
...
const roundedRectPath = (ctx: any, x: number, y: number, w: number, h: number, r: number): void => {
  ctx.beginPath()
  ctx.moveTo(x + r, y)
...
export const renderCanvasQuoteCard = async (input: QuoteCardInput): Promise<Buffer> => {
  let canvasMod: any
  try {
...
export const buildQuoteCard = async (input: QuoteCardInput): Promise<Buffer> => {
  try {
    return await fetchLyoQuoteCard(input)
  } catch (primaryError) {
    console.error(
      "[qc] lyo.su falló, usando respaldo con canvas:",
