...
type GachaState = {
  lastCharacters: Record<string, LastRolled>
  claimMessages: Record<string, string>
}

export type GachaRuntime = {
  botJid: string
  botType?: string
  mainBotJid?: string
}
...
const getSql = <T = any>(sql: string, params: unknown[] = []): Promise<T | undefined> => {
  return new Promise((resolve, reject) => {
...
export const jidTag = (jid: string): string => `@${normalizeJid(jid).split("@")[0]}`

export const getMentionedJid = (mctx: types.MessageContext): string | undefined => {
  return (mctx.message.mentioned?.[0] || mctx.message.mentionedJid?.[0]) as string | undefined
}

export const removeMentionFromArgs = (args: string[], jid?: string): string => {
...
export const randomOf = <T>(items?: T[]): T | undefined => {
  if (!items?.length) return undefined
  return items[Math.floor(Math.random() * items.length)]
}

export const getCharacterImage = (character: Character): string | undefined => {
  return randomOf(character.img?.filter(Boolean)) || character.image
}

export const getCharacterVideo = (character: Character): string | undefined => {
  return randomOf(character.vid?.filter(Boolean))
}

export const gachaTitle = (title: string, subtitle?: string): string => {
  let text = `» ˚₊· ͟͟͞͞➳❥ *${title}*\n`
  if (subtitle) text += `> ${subtitle}\n`
  return text
}

export const usageBlock = (title: string, examples: string[]): string => {
...
export const getDisplayName = async (
  ...
  if (mctx.is_group) {
    ...
    if (nick) return cleanInput(nick)
  }
...
export const formatUser = async (
  ...
  if (mode === "tag") return { text: jidTag(jid), mentions: [jid] }
  return { text: await getDisplayName(wss, mctx, jid), mentions: [] }
}

export const getRuntimeGacha = (bot: Partial<types.BotDocument>, group?: Partial<types.GroupDocument>): GachaDatabaseIndividual => {
...
export class GachaDatabaseIndividual {
  ...
  private mainBotJid: string
  private charactersFile: string
  private haremFile: string
  private stateFile: string

  constructor(botJid: string, botType = "main", mainBotJid?: string) {
  ...
  private initializeFiles(): void {
    if (!fs.existsSync(this.charactersFile)) writeJson(this.charactersFile, [])
    if (!fs.existsSync(this.haremFile)) writeJson(this.haremFile, [])
    if (!fs.existsSync(this.stateFile)) writeJson(this.stateFile, { lastCharacters: {}, claimMessages: {} })
  }

  loadCharacters(): Character[] {
    return readJson<Character[]>(this.charactersFile, [])
  }
  ...
  loadHarem(): HaremEntry[] {
    return readJson<HaremEntry[]>(this.haremFile, [])
  }

  saveHarem(harem: HaremEntry[]): void {
    writeJson(this.haremFile, harem)
  }

  loadState(): GachaState {
    const state = readJson<GachaState>(this.stateFile, { lastCharacters: {}, claimMessages: {} })
  ...
  addCharacter(character: Partial<Character>): boolean {
  ...
  searchCharacters(query: string, limit = 10): Character[] {
    ...
    const scored = characters
      .map((char) => {
        ...
        if (source === term) score += 35
        if (source.includes(term)) score += 20
        return { char, score }
      })
  ...
  findCharacter(query: string): Character | null {
  ...
  getRandomCharacter(): Character | null {
    const characters = this.loadCharacters()
    if (!characters.length) return null
    return characters[Math.floor(Math.random() * characters.length)]
  }

  getCharacterOwner(characterId: string): string | null {
    const entry = this.loadHarem().find((item) => item.character_id === characterId)
    return entry?.user_id || null
  }

  getOwnerEntry(characterId: string): HaremEntry | null {
  ...
  getUserCharacters(userId: string): Character[] {
    ...
    const characters = this.loadCharacters()
    return harem.map((entry) => characters.find((char) => char.id === entry.character_id)).filter(Boolean) as Character[]
  }

  getUserEntries(userId: string): Array<{ entry: HaremEntry; character: Character }> {
    const characters = this.loadCharacters()
  ...
  getUserStats(userId: string): { count: number; value: number; votes: number; saleCount: number } {
  ...
  claimCharacter(userId: string, characterId: string): { ok: boolean; reason?: string } {
    const harem = this.loadHarem()
    if (harem.some((entry) => entry.character_id === characterId)) return { ok: false, reason: "already" }
  ...
  deleteUserCharacter(userId: string, characterId: string): boolean {
    const harem = this.loadHarem()
    const next = harem.filter((entry) => !(entry.user_id === userId && entry.character_id === characterId))
    if (next.length === harem.length) return false
    this.saveHarem(next)
    return true
  }

  transferCharacter(fromUserId: string, toUserId: string, characterId: string): boolean {
    const harem = this.loadHarem()
  ...
  transferAll(fromUserId: string, toUserId: string): number {
    const harem = this.loadHarem()
    let count = 0
  ...
  tradeCharacters(userId: string, ownCharacterId: string, otherCharacterId: string): { ok: boolean; otherUser?: string; reason?: string } {
    const harem = this.loadHarem()
  ...
  setSale(userId: string, characterId: string, price: number): boolean {
    const harem = this.loadHarem()
  ...
  removeSale(userId: string, characterId: string): boolean {
    const harem = this.loadHarem()
    const entry = harem.find((item) => item.user_id === userId && item.character_id === characterId)
    if (!entry || !entry.sale_price) return false
    entry.sale_price = null
    this.saveHarem(harem)
    return true
  }

  getSales(page = 1, limit = 10): { items: SaleItem[]; total: number; pages: number } {
    const characters = this.loadCharacters()
  ...
  async buyCharacter(groupJid: string, buyerId: string, characterId: string): Promise<{ ok: boolean; reason?: string; price?: number; seller?: string }> {
    const harem = this.loadHarem()
    const entry = harem.find((item) => item.character_id === characterId && Number(item.sale_price || 0) > 0)
    if (!entry) return { ok: false, reason: "not_sale" }
    if (entry.user_id === buyerId) return { ok: false, reason: "self" }

    const price = Number(entry.sale_price || 0)
    const balance = await this.getBalance(groupJid, buyerId)
    if (balance < price) return { ok: false, reason: "money", price, seller: entry.user_id }
  ...
  saveLastCharacter(character: Character, groupJid: string): void {
    const state = this.loadState()
    state.lastCharacters[groupJid] = { character, timestamp: Date.now(), group_jid: groupJid }
    this.saveState(state)
  }

  getLastRolledCharacter(groupJid: string): Character | null {
    const state = this.loadState()
  ...
  clearLastRolledCharacter(groupJid: string): void {
    const state = this.loadState()
    delete state.lastCharacters[groupJid]
    this.saveState(state)
  }

  setClaimMessage(userId: string, message: string): void {
    const state = this.loadState()
    state.claimMessages[userId] = cleanInput(message).slice(0, 500)
    this.saveState(state)
  }

  deleteClaimMessage(userId: string): void {
    const state = this.loadState()
    delete state.claimMessages[userId]
    this.saveState(state)
  }

  getClaimMessage(userId: string): string {
  ...
  getTopCharacters(limit: number, offset = 0): Character[] {
    return this.loadCharacters().sort((a, b) => Number(b.value || 0) - Number(a.value || 0)).slice(offset, offset + limit)
  }

  getVoteTop(limit = 10): Character[] {
    return this.loadCharacters().sort((a, b) => Number(b.votes || 0) - Number(a.votes || 0)).slice(0, limit)
  }

  getTopOwners(limit = 10): Array<{ jid: string; count: number; value: number }> {
    const characters = this.loadCharacters()
    const values = new Map<string, { jid: string; count: number; value: number }>()
  ...
  voteCharacter(characterId: string): { ok: boolean; newValue?: number; increment?: number; votes?: number } {
    const characters = this.loadCharacters()
  ...
  async getBalance(groupJid: string, userId: string): Promise<number> {
    await this.ensureGroupUser(groupJid, userId)
    const row = await getSql<{ money?: number }>("SELECT money FROM group_users WHERE group_jid = ? AND user_jid = ?", [groupJid, userId])
    return Number(row?.money || 0)
  }

  async addMoney(groupJid: string, userId: string, amount: number): Promise<void> {
    await this.ensureGroupUser(groupJid, userId)
  ...
  async ensureGroupUser(groupJid: string, userId: string): Promise<void> {
    await runSql(
      `INSERT OR IGNORE INTO group_users (group_jid, user_jid, money, money_deposited) VALUES (?, ?, 0, 0)`,
      [groupJid, userId],
    )
  }

  async fetchCharactersBatch(batchSize: number): Promise<{ added: number; skipped: number; nextPage: number }> {
    return this.fetchCharactersContinue(1, batchSize)
  }

  async fetchCharactersContinue(startPage: number, batchSize: number): Promise<{ added: number; skipped: number; nextPage: number }> {
    let added = 0
    let skipped = 0
    ...
    while (added + skipped < Math.max(1, batchSize)) {
      ...
      try {
        ...
        for (const item of chars) {
          if (added + skipped >= Math.max(1, batchSize)) break
  ...
  async searchAnime(query: string): Promise<{ title: string; description: string; episodes?: number; status?: string; image?: string; characters: number } | null> {
  ...
  getHaremInfo(): { file: string; shared: boolean; mainBot?: string } {
    return { file: this.haremFile, shared: this.botType === "free", mainBot: this.botType === "free" ? this.mainBotJid : undefined }
  }
}

export type GachaPageParseResult = {
  page: number
  args: string[]
}

export type GachaPageSlice<T> = {
  page: number
  pages: number
  total: number
  start: number
...
export const normalizeGachaSearch = (value: string): string => {
  return String(value || "")
    .toLowerCase()
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
...
export const parseGachaPageArgs = (args: string[]): GachaPageParseResult => {
  const clean = [...args]
  let page = 1
...
export const paginateGacha = <T>(items: T[], inputPage = 1, pageSize = 10): GachaPageSlice<T> => {
...
export const gachaPercent = (value: number, total: number): string => {
  if (!total) return "0%"
  return `${Math.round((value / total) * 100)}%`
}

export const gachaCompactHeader = (title: string, subtitle?: string): string => {
  let text = `✿ *${title}*`
  if (subtitle) text += ` ${subtitle}`
  return text
}

export const gachaPageFooter = (page: number, pages: number): string => {
  return `\n\n▏▱ Página *${page}* de *${pages}*`
}

export const gachaPageHint = (prefix: string, command: string): string => {
