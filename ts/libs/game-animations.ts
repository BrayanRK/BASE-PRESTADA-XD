...
const loadCanvas = async (): Promise<any> => {
  try {
...
const topng = async (canvas: any): Promise<Buffer> => {
  const result = canvas.toBuffer?.("image/png")
  if (result instanceof Promise) return result
  if (Buffer.isBuffer(result) && result.length) return result
  return canvas.encode("png")
}

const roundedRect = (ctx: any, x: number, y: number, w: number, h: number, r: number): void => {
  ctx.beginPath()
  ctx.moveTo(x + r, y)
...
const drawDie = (ctx: any, x: number, y: number, size: number, value: number): void => {
...
const randomDieFace = (): number => Math.floor(Math.random() * 6) + 1

const REEL_COLORS = ["#e74c3c", "#f39c12", "#9b59b6", "#2ecc71", "#3498db", "#e67e22"]
const REEL_LABELS = ["7", "$", "♦", "★", "♠", "♥"]

const drawReel = (ctx: any, x: number, y: number, w: number, h: number, symbolIndex: number, spinning = false): void => {
...
const emojiToIndex = (emoji: string): number => {
...
const randomReelIndex = (): number => Math.floor(Math.random() * REEL_LABELS.length)

const framesToVideo = async (frames: Buffer[], fps: number): Promise<Buffer> => {
  ...
  try {
    await Promise.all(
      frames.map((frame, i) => fs.writeFile(path.join(dir, `f${String(i).padStart(4, "0")}.png`), frame)),
    )
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      child.on("close", (code) => {
        if (code !== 0) return reject(new Error(errText || `ffmpeg salió con código ${code}`))
        resolve()
      })
    })

    const data = await fs.readFile(outputPath)
    if (!data.length) throw new Error("ffmpeg no generó el video.")
    return data
  } finally {
    await cleanup()
  }
...
export const renderDiceAnimation = async (die1: number, die2: number): Promise<Buffer> => {
...
export const renderSlotsAnimation = async (a: string, b: string, c: string): Promise<Buffer> => {
  const { createCanvas } = await loadCanvas()
