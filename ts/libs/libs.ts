...
export const getBotType = (type: types.TypeBots): string => {
...
export class Command {
  static loaded = new Map<string, types.Command>()

  private static async loadFromFolder(folderPath: string): Promise<number> {
    let count = 0
    try {
      const files = await fsp.readdir(folderPath)

      for (const filename of files) {
        if (!/m?(t|j)s$/.test(filename)) continue
  ...
  static load = async (): Promise<void> => {
    try {
      ...
      const items = (await fsp.readdir(commandsPath, { withFileTypes: true })).sort((a, b) => {
        if (a.name === "animes_gen") return 1
        if (b.name === "animes_gen") return -1
  ...
  static reload = async (filename: string): Promise<void> => {
    if (!/m?(t|j)s$/.test(filename)) return
  ...
  static get = (key: string): types.Command | undefined => {
...
const loadSingleSession = async (id: string): Promise<"loaded" | "corrupted" | "skipped"> => {
  ...
  try {
    ...
    for (let i = 0; i < sessionsList.length; i += SESSIONS_BATCH_SIZE) {
      ...
      for (const result of results) {
        ...
        else if (result === "corrupted") corrupted++
        else skipped++
      }
    }
    ...
    if (corrupted > 0) {
      console.log(
        `[System] Sesiones recargadas: ${loaded} cargadas, ${corrupted} corruptas eliminadas, ${skipped} omitidas (${elapsed}ms).`,
      )
    }
  } catch (error) {
