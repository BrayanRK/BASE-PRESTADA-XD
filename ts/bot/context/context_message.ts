...
const asWAMessage = (message: unknown): baileys.WAMessage => message as baileys.WAMessage
...
const cleanDisplayName = (name?: string | null, fallback = ""): string => {
...
const getParticipantName = (metadata: any, jid?: string | null): string => {
  const participant = metadata?.participants?.find((p: any) => sameJid(p?.id || p?.jid, jid))
  return cleanDisplayName(
    participant?.name || participant?.notify || participant?.verifiedName || participant?.pushName,
    "",
  )
}

const formatJidNumber = (jid?: string | null): string => {
...
const normalizeLogText = (value?: string | null, maxLength = 650): string => {
...
const wrapLogText = (text: string, maxLineLength = 78): string[] => {
...
const getCommandPreview = (text?: string | null): string => {
...
const printMessageLog = (mctx: types.MessageContext): void => {
...
const deleteJidCandidates = (jid?: string | null): string[] => {
...
const deleteKeyCandidates = (
  ...
  for (const jid of [baseKey.participant, fallbackParticipant, senderJid]) {
    for (const candidate of deleteJidCandidates(jid)) participants.add(candidate)
  }

  const keys: any[] = [{ ...baseKey, remoteJid, id }]
...
const forceDeleteMessage = async (
  wss: types.WASocket,
  chatJid: string,
  baseKey: any,
  senderJid?: string | null,
...
const getCachedGroupMetadata = async (wss: types.WASocket, groupJid: string): Promise<baileys.GroupMetadata | null> => {
...
const cacheKey = (remoteJid?: string | null, id?: string | null, participant?: string | null): string => {
  return [safeNormalizeJid(remoteJid), id || "", safeNormalizeJid(participant)].join("|")
}

const rememberMessage = (message: baileys.WAMessage): void => {
...
const getCachedMessage = (remoteJid?: string | null, id?: string | null, participant?: string | null): baileys.WAMessage | null => {
  if (!remoteJid || !id) return null

  return (
    messageCache.get(cacheKey(remoteJid, id, participant)) ||
    messageCache.get(cacheKey(remoteJid, id)) ||
    null
  )
}

export const contextMessage = async (message: baileys.WAMessage, wss: types.WASocket) => {
  if (!message || !message.message) return null;
  const content = unwrapMessageContent(message.message)
  const mediaSourceMessage = { ...message, message: content } as baileys.WAMessage
  ;(mediaSourceMessage as any).__zetaOriginal = message
  rememberMessage(mediaSourceMessage)
  const mctx = <types.MessageContext>{
      sender: {
          jid: '',
          name: '',
      },
  ...
  mctx.sender.jid = safeNormalizeJid(message.key.fromMe ? ((wss.user as any)?.lid || wss.user?.id || message.key.remoteJid) : message.participant || message.key.participant || message.key.remoteJid);
  ...
  mctx.message.type = Object.keys(content).find((v: baileys.MessageType) => v !== 'senderKeyDistributionMessage' && v !== 'messageContextInfo') || 'conversation';
  mctx.message.from_me = message.key.fromMe;
  mctx.message.id = message.key.id;
  mctx.message.original = message;
  const context: baileys.proto.IContextInfo = content[mctx.message.type]?.contextInfo || null;
  ...
  mctx.me.jids.lid = safeNormalizeJid((wss.user as any)?.lid || wss.user?.id);
  mctx.conn = wss;
  mctx.reply = async (text: string, server?: 'lid' | 's.whatsapp.net') => {
      try {
        const botJid = (wss as any).user?.id
          ? (wss as any).user.id.split(":")[0] + "@s.whatsapp.net"
          : ""
        if (botJid && isTextOnlyPayload({ text })) {
          ...
          if (cfg.enabled && cfg.title && cfg.image_url) {
            ...
            return wss.sendMessage(mctx.chat.jid, wrapped as any, { quoted: asWAMessage(message) })
          }
        }
      } catch (pcoErr) {
  ...
  mctx.edit = async (text: string, server?: 'lid' | 's.whatsapp.net') => {
  ...
  if (context?.quotedMessage) {
      ...
      const quoted = {
          ...
          key: {
              ...
              fromMe: Boolean(cachedQuotedMessage?.key.fromMe) || quotedParticipant === safeNormalizeJid((wss.user as any)?.lid || wss.user?.id),
      ...
      const quotedSenderName = await resolveDisplayName(wss, quoted.key.participant || quoted.key.remoteJid, {
          fallbackName: quoted.key.fromMe
              ...
              : cachedQuotedMessage?.pushName || (cachedQuotedMessage as any)?.verifiedBizName,
      ...
      mctx.quoted = <types.MessageContext>{
          sender: {
      ...
      mctx.quoted.message.type = Object.keys(quoted.message).find((v: baileys.MessageType) => v !== 'senderKeyDistributionMessage' && v !== 'messageContextInfo') || 'conversation';
      ...
      mctx.quoted.edit = async (text: string) => {
      ...
      mctx.quoted.react = async (emogi: string) => {
          try {
              return await wss.sendMessage(mctx.chat.jid, {
                  react: {
                      key: quoted.key,
