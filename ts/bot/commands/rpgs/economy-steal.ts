  ...
  execute: async (_, { mctx, group, bot, usedPrefix }) => {
    ...
    try {
      ...
      if (success) {
        conn.serialize(() => {
          conn.run(
            `UPDATE group_users SET money = money + ?, last_robbery_ago = ? WHERE group_jid = ? AND user_jid = ?`,
            [amount, now, scopedGroupJid, mctx.sender.jid],
          )
      ...
      conn.run(
        `UPDATE group_users SET money = money - ?, last_robbery_ago = ? WHERE group_jid = ? AND user_jid = ? AND money >= ?`,
        [failPenalty, now, scopedGroupJid, mctx.sender.jid, failPenalty],
      )
