...
const calculateDamage = (attacker: any, defender: any) => {
  ...
  if (dmg < 1) dmg = 1
  const adv = attacker.types.some((t: string) => ADVANTAGES[t]?.includes(defender.types[0])) ? 1.5 : 1
  const weak = attacker.types.some((t: string) => WEAKNESSES[t]?.includes(defender.types[0])) ? 0.5 : 1
          ...
          execute: async (wss, mcx) => {
            try {
              ...
              if (/aceptar/i.test(args[0])) {
                ...
                await mcx.reply(
                  `*｢❀｣* Empezando la batalla entre @${mctx.sender.jid.split("@")[0]} *[${pokeA.name}]* y @${mentioned.split("@")[0]} *[${pokeB.name}]*.`,
                )
                ...
                await mcx.reply(
                  "*｢✧｣* Responde con *aceptar <pokémon>* para iniciar la batalla o *rechazar* para cancelarla.",
                )
                return
              }
            } catch (error) {
