...
const colorMap: Record<string, RouletteColor> = {
  red: "red",
  rojo: "red",
  black: "black",
  negro: "black",
...
const getColor = (value?: string): RouletteColor | null => colorMap[String(value ?? "").trim().toLowerCase()] || null
...
const command: types.Command = {
  ...
  execute: async (_, { mctx, args, group, bot, usedPrefix }) => {
    ...
    const resultColor: RouletteColor | "green" = number === 0 ? "green" : number % 2 === 0 ? "black" : "red"
    ...
    try {
      ...
      conn.run(
        ...
        [delta, scopedGroupJid, mctx.sender.jid, won ? 0 : amount],
      )
