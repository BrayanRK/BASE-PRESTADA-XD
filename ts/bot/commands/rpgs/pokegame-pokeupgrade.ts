  ...
  execute: async (wss, { mctx, group, args, bot }) => {
    ...
    return new Promise<void>((resolve) => {
      try {
        ...
        conn.get(
          `SELECT * FROM user_pokemon WHERE user_jid = ? AND group_jid = ? AND pokemon_name = ?`,
          [mctx.sender.jid, scopedGroupJid, args[0]],
          (err, pokemonRow: any) => {
            if (err) {
            ...
            const currentValue: number = baseStats[args[1]]
            const enhancersRequired = Math.floor(currentValue / 10) + 1

            if (groupUser.enhancers < enhancersRequired) {
              mctx.reply(
                `*｢✧｣* Necesitas *${enhancersRequired}* potenciadores para mejorar el *${args[1]}* de *${pokemon.pokemon_name}*.`,
              )
              resolve()
              return
            }
            ...
            conn.serialize(() => {
              ...
              conn.run(
                `UPDATE group_users SET enhancers = enhancers - ? WHERE group_jid = ? AND user_jid = ?`,
                [enhancersRequired, scopedGroupJid, mctx.sender.jid],
                async (err) => {
                  if (err) {
                    console.error("[PokeUpgrade] Error:", err)
                    ...
                    await mctx.reply(
                      `*｢❀｣* Mejoraste el *${args[1]}* de *${pokemon.pokemon_name}* a *${currentValue + 1}* y gastaste *${enhancersRequired}* potenciadores.`,
                    )
                  }
                  resolve()
                },
