...
const jidNumber = (jid?: string | null): string => stripJidDevice(jid).split("@")[0].replace(/[^0-9]/g, "")

const socketCard = (title: string, lines: string[]): string =>
  [`「✧」 ${title}`, ...lines.map((l) => `◇ ${l}`)].join("\n")

const promptForNumber = (usedPrefix: string): string =>
  socketCard("Ingresa el socket a revertir a su tipo original.", [
    `Uso › ${usedPrefix}unsetprimary 595981902212`,
    `Uso › ${usedPrefix}unsetprimary @bot`,
    "También sirve respondiendo un mensaje del socket.",
  ])

const findActiveRuntimeSocket = (
  identities: string[],
): { active: types.BotData; runtimeJid: string; jid: string; number: string } | null => {
  for (const identity of identities) {
    const targetJid = stripJidDevice(identity)
    const targetNumber = jidNumber(identity)
    if (!targetJid && !targetNumber) continue
    ...
    for (const [runtimeJidRaw, runtimeBot] of Bot.bots) {
      ...
      const phoneNumber =
        jidNumber((runtimeBot.wss as any)?.user?.id) || jidNumber(botJid) || jidNumber(runtimeJid)
      ...
      if (!numberMatch && !jidMatch) continue
...
const getTargetIdentity = (mctx: types.MessageContext, args: string[]): string => {
...
const revertRuntimeType = (
  number: string,
  targetJid: string,
  restoredType: types.TypeBots,
  parentBotJid: string,
): void => {
  for (const [runtimeJid, runtimeBot] of Bot.bots) {
    const match =
      stripJidDevice(runtimeJid) === targetJid ||
...
const command: types.Command = {
  ...
  execute: async (wss, { mctx, args, group, usedPrefix, groupMetadata }) => {
    ...
    if (!activeTarget) {
      ...
      const candidates = [resolved?.lidJid, resolved?.phoneJid, resolved?.phoneNumber, resolved?.bestJid].filter(
        (v): v is string => Boolean(v),
      )
      if (candidates.length) activeTarget = findActiveRuntimeSocket(candidates)
    }

    if (!activeTarget) {
      await mctx.reply(
        socketCard("Ese socket no está activo.", [
    ...
    const currentType = (activeTarget.active.bot_type || "free") as types.TypeBots

    if (currentType !== "main") {
      await mctx.reply(
        socketCard("Ese socket ya no es oficial.", [
    ...
    const runtimeOriginalType = activeTarget.active.original_type as types.TypeBots | undefined
    const savedBot = await database.Bots.find(activeTarget.jid).catch(() => null)

    // Buscar original_type en bot_sessions también
    const sessionRow = await new Promise<{ original_type?: string } | null>((resolve) => {
      getConnection().get(
        `SELECT original_type FROM bot_sessions WHERE bot_number = ? OR bot_jid = ? LIMIT 1`,
        [activeTarget!.number || number, activeTarget!.jid],
        (err, row: any) => resolve(err ? null : (row ?? null)),
      )
    })

    const restoredType: types.TypeBots =
      runtimeOriginalType && runtimeOriginalType !== "main"
        ? runtimeOriginalType
        : sessionRow?.original_type && sessionRow.original_type !== "main"
          ? (sessionRow.original_type as types.TypeBots)
          : (savedBot?.bot_type !== "main" ? savedBot?.bot_type as types.TypeBots : undefined) ?? "free"
    ...
    if (!updated) {
      await mctx.reply(
        socketCard("No pude actualizar el socket.", [
          `Bot › @${activeTarget.number || number}`,
    ...
    await new Promise<void>((resolve) => {
      getConnection().run(
        `UPDATE bot_sessions SET bot_type = ?, parent_bot_jid = ?, original_type = '', last_seen = CURRENT_TIMESTAMP WHERE bot_number = ? OR bot_jid = ?`,
        [restoredType, parentBotJid, activeTarget!.number || number, activeTarget!.jid],
        () => resolve(),
      )
    })
    ...
    await mctx.reply(
      socketCard("Socket revertido a tipo original.", [
        `Bot › @${activeTarget.number || number}`,
        `Nombre › ${botName}`,
