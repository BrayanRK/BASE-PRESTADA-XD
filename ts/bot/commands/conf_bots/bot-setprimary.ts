...
const runSql = <T = any>(sql: string, params: unknown[] = []): Promise<T[]> => {
...
const runUpdate = (sql: string, params: unknown[] = []): Promise<void> => {
  return new Promise((resolve, reject) => {
...
type TargetSocket = {
  ...
  active: types.BotData
  runtimeJid: string
  jid: string
  number: string
}

const runtimeSocketIds = (runtimeJidRaw: string, runtimeBot: types.BotData): string[] => {
  const rawValues = [
    runtimeJidRaw,
    runtimeBot.bot_jid,
    (runtimeBot.wss as any)?.user?.id,
    (runtimeBot.wss as any)?.user?.lid,
  ]

  return Array.from(new Set(rawValues.map(stripJidDevice).filter(Boolean)))
}

const findActiveRuntimeSocket = (identities: string[]): { active: types.BotData; runtimeJid: string; jid: string; number: string } | null => {
  for (const identity of identities) {
    ...
    for (const [runtimeJidRaw, runtimeBot] of Bot.bots) {
      ...
      const phoneNumber = jidNumber((runtimeBot.wss as any)?.user?.id) || jidNumber(botJid) || jidNumber(runtimeJid)
...
const findSession = async (number: string, jid: string): Promise<SessionRow | null> => {
  const rows = await runSql<SessionRow>(
    `SELECT bot_jid, bot_number, owner_jid, user_jid, bot_type, parent_bot_jid
     FROM bot_sessions
...
const setRuntimeAsMain = (number: string, targetJid: string): void => {
...
const updateSessionAsMain = async (number: string, targetJid: string): Promise<void> => {
  await runUpdate(
    `UPDATE bot_sessions
...
const updateBotAsMain = async (target: TargetSocket): Promise<types.BotDocument | null> => {
...
const command: types.Command = {
  ...
  execute: async (wss, { mctx, args, group, usedPrefix, groupMetadata }) => {
    ...
    const beforeType = (target.active.bot_type || target.bot?.bot_type || target.session?.bot_type || "free") as types.TypeBots
    if (beforeType === "main") {
      await mctx.reply(
        socketCard("Ese socket ya es oficial.", [
    ...
    await mctx.reply(
      socketCard("Socket activo convertido a oficial.", [
        `Bot › @${target.number || number}`,
        `Nombre › ${botName}`,
