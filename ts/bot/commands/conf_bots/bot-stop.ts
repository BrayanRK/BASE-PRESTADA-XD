...
const box = (title: string, lines: string[]): string => [`╭─〔 ${title} 〕`, ...lines.map((line) => `│ ${line}`), "╰────────────"].join("\n")

const resolveTargetBotType = async (targetNumber: string, targetJid: string): Promise<types.TypeBots | ""> => {
  const activeEntry = Array.from(Bot.bots.entries()).find(([jid, data]) => normalizeSocketNumber(jid || data.bot_jid) === targetNumber)
  if (activeEntry?.[1]?.bot_type) return activeEntry[1].bot_type as types.TypeBots

  const persisted = await database.Bots.find(targetJid).catch(() => null)
  if (persisted?.bot_type) return persisted.bot_type as types.TypeBots

  const sessions = await BotPersistence.loadBots().catch(() => [])
  const saved = sessions.find((item: any) => normalizeSocketNumber(item.bot_number || item.bot_jid) === targetNumber)
  return (saved?.bot_type as types.TypeBots) || ""
}
...
const command: types.Command = {
  ...
  execute: async (_wss, { mctx, args, bot, userIsOwner }) => {
    ...
    setTimeout(() => {
      try {
        active?.[1]?.wss && (active[1].wss as any).end?.(undefined)
      } catch {}
      if (active) Bot.bots.delete(active[0])
    }, 800)
  },
}
