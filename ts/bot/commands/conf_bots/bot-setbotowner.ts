...
import { canConfigureSocket, denyFreeConfigMessage, normalizeJid, ownerIsConfigured, socketConfigOnlyMessage, socketUsage } from "../../../libs/socket-manager.js"
...
const stripJidDevice = (jid: string): string => cleanText(jid).split(":")[0].toLowerCase()
const jidNumber = (jid?: string | null): string => stripJidDevice(String(jid || "")).split("@")[0].replace(/[^0-9]/g, "")
const isRawJid = (value: string): boolean => /@(lid|s\.whatsapp\.net)$/i.test(stripJidDevice(value))
const isLidJid = (value?: string): boolean => /@lid$/i.test(stripJidDevice(String(value || "")))
const isPhoneJid = (value?: string): boolean => /@s\.whatsapp\.net$/i.test(stripJidDevice(String(value || "")))

const normalizeIdentityJid = (value?: string | null): string => {
...
const getPhoneNumberFromText = (text: string): string => {
...
const getExplicitIdentityJid = (mctx: types.MessageContext, args: string[]): string => {
  ...
  if (first === "me" || first === "yo") return mctx.sender.jid

  const namedIdentity = getNamedIdentityArg(args)
  if (namedIdentity) return namedIdentity

  const rawJidArg = args.find((arg) => isRawJid(arg)) || ""
  if (rawJidArg) return rawJidArg

  return ""
}

const getTypedName = (text: string): string =>
  cleanText(text)
...
const botText = (text: string): string => `「◈」 *${text}*`

const ownerIdUsage = (usedPrefix = "."): string =>
  botText(`Uso: ${usedPrefix}setbotowner @usuario 573161325891`)
...
const command: types.Command = {
  ...
  execute: async (wss, { mctx, args, bot, userIsBotOwner, usedPrefix, groupMetadata }) => {
    ...
    const updatePayload: Partial<types.BotDocument> = {
      owner_jid: targetJid,
      owner_lid: ownerLid,
      owner_pn: ownerPn,
