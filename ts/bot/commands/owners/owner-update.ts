...
const run = async (command: string, timeout = 120_000): Promise<string> => {
...
const safeRun = async (command: string, timeout = 120_000): Promise<string> => {
  try {
...
const fullHash = async (): Promise<string> => clean(await safeRun("git rev-parse HEAD", 20_000))

const hasPackageChanges = (changes: string): boolean => {
