...
const formatFlags = (command: types.Command): string => {
  const requires = Array.isArray(command.requires) ? command.requires : []
  const flags = Array.isArray(command.flags) ? command.flags : []
  const bits: string[] = []
  if (requires.includes("bot.owner")) bits.push("owner bot")
  if (requires.includes("owner.user")) bits.push("owner sistema")
  if (requires.includes("moderator.user")) bits.push("mod")
  if (flags.includes("only.groups")) bits.push("grupo")
  if (flags.includes("only.private")) bits.push("privado")
  if (command.hidden) bits.push("oculto")
  return bits.length ? ` _(${bits.join(" · ")})_` : ""
}

type OwnerSection = "control" | "sockets" | "premium" | "sistema" | "moderacion" | "rangos" | "otros"

const sectionMeta: Record<OwnerSection, { title: string; icon: string; desc: string }> = {
...
const ownerOnly = (command: types.Command): boolean => {
  const requires = Array.isArray(command.requires) ? command.requires : []
  return requires.includes("bot.owner") || requires.includes("owner.user")
}

const getSection = (command: types.Command): OwnerSection => {
  if (["getprem", "sumprem", "delprem", "checkprem", "myprem"].includes(command.name) || command.category === "premb") return "premium"
  if (command.category === "bot") return "sockets"
  if (["system", "update", "updatedb", "eval", "reload", "logout"].includes(command.name)) return "sistema"
  if (command.category === "moderation" || ["mute", "muteall", "autoadmin", "allchats"].includes(command.name)) return "moderacion"
  if (["setrange", "delrange", "addowner", "delowner"].includes(command.name)) return "rangos"
  if (command.category === "owner") return "control"
  return "otros"
}

const sectionBlock = (section: OwnerSection, commands: types.Command[], prefix: string): string => {
...
export default {
  ...
  execute: async (wss, { mctx, usedPrefix, bot, userIsPrimaryBotOwner }) => {
    const sections: Partial<Record<OwnerSection, types.Command[]>> = {}
    ...
    const ordered: OwnerSection[] = ["control", "sockets", "premium", "sistema", "moderacion", "rangos", "otros"]
