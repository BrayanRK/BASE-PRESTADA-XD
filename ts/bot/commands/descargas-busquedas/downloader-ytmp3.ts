...
const isYoutubeLink = (t: string): boolean => Boolean(extractYoutubeId(t))
...
export default {
  ...
  execute: async (wss, { mctx, args }) => {
    ...
    if (!url || !isYoutubeLink(url)) { await mctx.react("⚠️"); await mctx.reply("「⚠」 Envía un link de YouTube válido."); return }

    let video: Awaited<ReturnType<typeof resolveYoutubeInfo>> | null = null
    let err1: unknown, err2: unknown

    try { await mctx.react("🔎"); video = await resolveYoutubeInfo(url); await sendYoutubeInfoCard(wss, mctx, video, "audio") } catch {}


    try {
      await mctx.react("🎧")
      if (!video) video = await resolveYoutubeInfo(url)
    ...
    } catch (e) { err1 = e; console.error("[ytmp3] DV-YER falló:", e instanceof Error ? e.message : e) }
    ...
    try {
      ...
      await mctx.react("✅"); return
    } catch (e) { err2 = e; console.error("[ytmp3] EVOGB falló:", e instanceof Error ? e.message : e) }
