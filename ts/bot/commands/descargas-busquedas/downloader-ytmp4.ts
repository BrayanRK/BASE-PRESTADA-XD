...
export default {
  ...
  execute: async (wss, { mctx, args }) => {
    ...
    try { await mctx.react("🔎"); video = await resolveYoutubeInfo(url); await sendYoutubeInfoCard(wss, mctx, video, "video") } catch {}
    if (!video) { try { video = await resolveYoutubeInfo(url) } catch { await mctx.react("❌"); await mctx.reply("「✖」 No se pudo procesar ese link."); return } }
