...
const doneCaption = (caption?: string) => {
  const trimmed = caption?.trim()
  if (!trimmed) return "「◈」 *Descarga realizada*"
...
export default {
  ...
  execute: async (wss, { mctx, args }) => {
    ...
    if (!url || !isInstagramUrl(url)) { await mctx.react("⚠️"); await mctx.reply("「⚠」 Envía un link."); return }
    if (busy.has(mctx.sender.jid)) { await mctx.reply("「☊」 Descarga en proceso, espera a que termine."); return }
    ...
    try {
      ...
      try {
        ...
        const isVideo = /\.(mp4|webm|mov)([\?#]|$)/i.test(mediaUrl) || String((data as any).type).includes("video")
        ...
        await mctx.react("✅")
        return
      } catch (e) { console.error("[instagram] DV-YER falló:", e instanceof Error ? e.message : e) }
    } catch (e) { console.error("[instagram] Error:", e); await mctx.react("❌"); await mctx.reply("「✖」 No se pudo realizar la descarga.") }
    finally { busy.delete(mctx.sender.jid) }
  },
} as types.Command
