...
const short = (v: string, max = 900): string => v.length > max ? `${v.slice(0, max - 3)}...` : v
...
const isVideoLike = (url: string): boolean => /\.(mp4|mov|m4v|webm)(\?|#|$)/i.test(url) || /(?:v\d?\.pinimg\.com|pinimg\.com\/videos|video)/i.test(url)
  ...
  execute: async (wss, { mctx, args }) => {
    ...
    if (isPinterestUrl(query)) {
      try {
        ...
        if (!media) throw new Error("Sin video")
    ...
    try {
      ...
      const valid = items.filter((item) => {
        try { const u = evogbMediaUrl(item) || evogbLink(item); return Boolean(u && /^https?:\/\//i.test(u)) } catch { return false }
      })
      const item = pickRandom(valid)
      if (!item) throw new Error("Sin videos")
      let mediaUrl = ""
      try { mediaUrl = evogbMediaUrl(item) } catch { mediaUrl = evogbLink(item) }
      ...
      if (!mediaUrl || !isVideoLike(mediaUrl)) throw new Error("No hay video directo")
    ...
    try {
      ...
      if (!media) throw new Error("Sin videos")
