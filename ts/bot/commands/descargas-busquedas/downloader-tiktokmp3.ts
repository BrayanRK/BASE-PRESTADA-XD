...
export default {
  ...
  execute: async (wss, { mctx, args }) => {
    ...
    if (!url) { await mctx.react("⚠️"); await mctx.reply("「⚠」 Envía un link de TikTok."); return }
    try {
      await mctx.react("🎧")
      const data = await evogbTikTokMp3(url)
      await wss.sendMessage(mctx.chat.jid, { audio: { url: evogbMediaUrl(data) }, mimetype: "audio/mpeg", fileName: String((data as any).fileName || (data as any).filename || "tiktok.mp3") }, { quoted: mctx.message.original })
      await mctx.react("✅")
    } catch (e) {
      console.error("[tiktokmp3] Error:", e instanceof Error ? e.message : e)
      await mctx.react("❌"); await mctx.reply(`「✖」 ${evogbUserError(e, "No se pudo realizar la descarga.")}`)
    }
  },
} as types.Command
