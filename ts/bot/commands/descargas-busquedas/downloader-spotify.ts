...
const isSpotifyLink = (v: string) => /spotify\.com/i.test(v)
const mkCaption = (title: string, artist: string, album: string, url: string) =>
  ["「🎧」 Spotify DL", "", `✦ Título › ${title}`, `✦ Artista › ${artist}`, album && `✦ Álbum › ${album}`, url && `✦ Spotify › ${url}`].filter(Boolean).join("\n").slice(0, 1000)
...
export default {
  ...
  execute: async (wss, { mctx, args }) => {
    ...
    if (!input) { await mctx.react("⚠️"); await mctx.reply("「⚠」 Escribe un link o nombre de Spotify."); return }
    ...
    try {
      ...
      const t = tracks[0]
      if (t) { spotifyLink = dvyerLink(t) || spotifyLink; ytQuery = `${dvyerTitle(t)} ${dvyerAuthor(t, "")}`.trim() || input }
    } catch (e) { err1 = e }

    if (spotifyLink) {
      try {
        const data = await dvyerSpotify(spotifyLink)
        await wss.sendMessage(mctx.chat.jid, { audio: { url: dvyerMediaUrl(data) }, mimetype: "audio/mpeg", fileName: `${s((data as any).title) || "spotify"}.mp3` }, { quoted: mctx.message.original })
        await mctx.react("✅"); return
      } catch (e) { err1 = e; console.error("[spotify] DV-YER dl falló:", e instanceof Error ? e.message : e) }
    }
    ...
    try {
      ...
      const t = tracks[0]
      if (t) { spotifyLink = evogbLink(t) || spotifyLink; ytQuery = `${evogbTitle(t)} ${evogbAuthor(t, "")}`.trim() || input }
    } catch (e) { err2 = e }

    if (spotifyLink) {
      try {
        const data = await evogbSpotifyDl(spotifyLink)
        await wss.sendMessage(mctx.chat.jid, { audio: { url: evogbMediaUrl(data) }, mimetype: "audio/mpeg", fileName: String((data as any).fileName || (data as any).filename || `${s(ytQuery)}.mp3`) }, { quoted: mctx.message.original })
        await mctx.react("✅"); return
      } catch (e) { err2 = e; console.error("[spotify] EVOGB dl falló:", e instanceof Error ? e.message : e) }
    }
