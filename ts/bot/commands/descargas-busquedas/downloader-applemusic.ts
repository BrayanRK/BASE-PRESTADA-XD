...
const usage = (): string => "「⚠」 Envía un link o nombre de Apple Music."
const isAppleMusicLink = (v: string) => /music\.apple\.com/i.test(v)
  ...
  execute: async (wss, { mctx, args }) => {
    ...
    try {
      ...
      await wss.sendMessage(
        mctx.chat.jid,
        { audio: { url: fileUrl }, mimetype: "audio/mpeg", fileName: `${title}.mp3` },
