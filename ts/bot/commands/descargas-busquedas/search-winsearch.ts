...
import { dvyerWinSearch, dvyerTitle, dvyerAuthor, dvyerSize, dvyerLink, dvyerUserError } from "../../../libs/downloads.js"

const usage = (): string => "「⚠」 Escribe el nombre del programa."
const searchCaption = (caption?: string) => ["「◈」 *Búsqueda realizada*", caption?.trim()].filter(Boolean).join("\n\n")

export default {
  name: "winsearch",
  alias: [],
  description: "Busca programas disponibles para Windows.",
  ...
  execute: async (_wss, { mctx, args }) => {
    ...
    try {
      await mctx.react("🔎")
      const items = await dvyerWinSearch(query)
      if (!items.length) {
        await mctx.react("❌")
      ...
      console.error("[winsearch] Error:", error instanceof Error ? error.message : error)
      await mctx.react("❌")
