...
const usage = (): string => "「⚠」 Escribe qué buscar o pega un link de Pinterest."
const isUrl = (v: unknown): v is string => typeof v === "string" && /^https?:\/\//i.test(v.trim())
const looksImage = (url: string): boolean => /\.(jpg|jpeg|png|webp|gif)(\?|#|$)/i.test(url) || /pinimg\.com/i.test(url)
const looksBad = (url: string): boolean => /(?:logo|favicon|avatar|ads?|static\/img|pinterest\.com\/pinimg)/i.test(url)
const pickRandom = <T>(list: T[]): T | undefined => list.length ? list[Math.floor(Math.random() * list.length)] : undefined
const downloadCaption = (c?: string): string => ["「◈」 *Descarga realizada*", c?.trim()].filter(Boolean).join("\n\n")
const searchCaption = (c?: string): string => ["「◈」 *Búsqueda realizada*", c?.trim()].filter(Boolean).join("\n\n")

const pickEvogbImage = (item: any): string => {
  const candidates = [evogbThumb(item), item?.image, item?.imageUrl, item?.thumbnail, item?.url, item?.link].filter(isUrl) as string[]
...
export default {
  ...
  execute: async (wss, { mctx, args }) => {
      ...
      try {
        ...
        await wss.sendMessage(mctx.chat.jid,
          media.type === "video"
            ...
            : { image: { url: media.url }, caption: downloadCaption(media.caption) },
    ...
    try {
      ...
      if (!media) throw new Error("Sin imágenes")
      await wss.sendMessage(mctx.chat.jid,
        media.type === "video"
          ? { video: { url: media.url }, caption: searchCaption(media.caption), mimetype: "video/mp4" }
          : { image: { url: media.url }, caption: searchCaption(media.caption) },
        { quoted: mctx.message.original })
      await mctx.react("✅")
    } catch (e) {
