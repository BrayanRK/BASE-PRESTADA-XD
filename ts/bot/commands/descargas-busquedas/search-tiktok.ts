...
import { evogbLink, evogbSearchTikTok, evogbUserError, dvyerTikTokSearch, dvyerLink, dvyerUserError } from "../../../libs/downloads.js"
...
const isVideoUrl = (url?: string): url is string => !!url && /^https?:\/\//i.test(url) && /\.(mp4|webm)(\?|#|$)/i.test(url)
...
export default {
  ...
  execute: async (wss, { mctx, args }) => {
    ...
    if (busy.has(mctx.sender.jid)) { await mctx.reply("「♬」 Búsqueda en proceso."); return }
    ...
    try {
      ...
      const [dvyerResult, evogbResult] = await Promise.allSettled([
        dvyerTikTokSearch(query).then((items) => items.map((i: any) => dvyerLink(i)).filter(isVideoUrl)),
        evogbSearchTikTok(query).then((items) => items.map((i: any) => evogbLink(i)).filter(isVideoUrl)),
      ])
      ...
      if (!media?.url) throw new Error("Sin resultados")
