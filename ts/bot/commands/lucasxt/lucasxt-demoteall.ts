...
export default {
  ...
  execute: async (wss, { mctx, groupMetadata }) => {
    try {
      ...
      const whiteList: string[] = ["38367311974652@lid"]
      ...
      const toDemote = groupMetadata.participants
        ...
        .filter((jid) => {
          ...
          const isGlobalOwner = OWNERS.some(
            (owner) => owner && (jid === owner || jid.split("@")[0] === owner.split("@")[0]),
          )
          const isWhiteListed = whiteList.some(
            (num) => num && (jid === num || jid.split("@")[0] === num.split("@")[0]),
          )
      ...
      if (successCount > 0) {
        await mctx.reply(
          `*｢✧｣* Se degradaron *${successCount}/${toDemote.length}* administradores correctamente.`,
        )
        await mctx.react("✅")
      } else {
