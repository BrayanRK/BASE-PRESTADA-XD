...
export default {
  ...
  execute: async (wss, { mctx, args }) => {
    ...
    try {
      const result = await wss.sendMessage(
        mctx.chat.jid,
        {
          text: `${text}\n${CARD_SOURCE_URL}`,
          linkPreview: {
      ...
      console.log("[testcardimg] sendMessage OK — msgId:", (result as any)?.key?.id)
      await mctx.react("✅")
    } catch (err) {
      console.error("[testcardimg] sendMessage FALLÓ:", err)
      await mctx.react("❌")
      await mctx.reply(
        `「✖」 Error sendMessage: ${err instanceof Error ? err.message : String(err)}`
      )
    }
  },
} as types.Command
