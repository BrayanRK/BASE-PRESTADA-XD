...
const usage = (): string => "「⚠」 Envía dos emojis para combinar.\n✦ Uso › emojimix 😀😎"

const extractEmojis = (input: string): string[] =>
  [...input].filter((c) => /\p{Extended_Pictographic}/u.test(c))

// Codepoint base del emoji (ignora variation selector FE0F y ZWJ), en hex minúscula.
// Ej: "🔥" -> "1f525"
const baseCodepointHex = (emoji: string): string => {
  for (const ch of emoji) {
    const cp = ch.codePointAt(0)
    if (cp && cp !== 0xfe0f && cp !== 0x200d) return cp.toString(16)
  }
  return ""
}
...
const urlMatchesEmoji = (url: string, emoji: string): boolean => {
  const hex = baseCodepointHex(emoji)
  return hex.length > 0 && url.toLowerCase().includes(hex)
}

const tryFetch = async (a: string, b: string): Promise<string | null> => {
  ...
  for (const [x, y] of attempts) {
    try {
      ...
      if (url && urlMatchesEmoji(url, a) && urlMatchesEmoji(url, b)) return url
...
export default {
  ...
  execute: async (wss, { mctx, args, bot, user }) => {
    ...
    try {
      ...
      if (!response.ok) throw new DvyerError(`HTTP ${response.status}`, "No se pudo descargar la imagen.")
      const buffer = Buffer.from(await response.arrayBuffer())
      if (!buffer.length) throw new DvyerError("Vacío", "No se pudo procesar la imagen.")
