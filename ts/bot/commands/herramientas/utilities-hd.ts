...
import {
  dvyerImageHD, dvyerMediaUrl, dvyerUserError,
  evogbMediaUrl, evogbUpscale, evogbUserError,
  findFirstUrl,
} from "../../../libs/downloads.js"
import { downloadMediaBuffer } from "../../../libs/media.js"

const usage = (): string => "「⚠」 Responde a una imagen o envía una URL."
...
const uploadBuffer = async (buffer: Buffer, mimetype: string): Promise<string> => {
  ...
  let json: any
  try { json = JSON.parse(text) } catch { throw new Error(`tmpfiles no-JSON: ${text.slice(0, 100)}`) }

  const raw: string = json?.data?.url || json?.url || ""
  if (!raw) throw new Error(`tmpfiles sin URL: ${text.slice(0, 150)}`)

  return raw.replace("tmpfiles.org/", "tmpfiles.org/dl/")
}

const getImageUrl = async (mctx: types.MessageContext, argsText: string): Promise<string> => {
  const directUrl =
  ...
  if (!source?.download) throw new Error("Sin imagen")

  const mimetype = source.message.mimetype || ""
  if (mimetype && !/^image\//i.test(mimetype)) throw new Error("No es imagen")
