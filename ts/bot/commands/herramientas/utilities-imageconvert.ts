...
const usage = (): string => "「⚠」 Responde a una imagen o envía una URL y formato. Ej: .imgconvert webp"
  ...
  execute: async (wss, { mctx, args }) => {
    try {
      ...
      await wss.sendMessage(
        mctx.chat.jid,
        { document: { url: fileUrl }, fileName: `converted.${format}`, mimetype: `image/${format === "jpg" ? "jpeg" : format}`, caption: "「◈」 *Imagen convertida*" },
