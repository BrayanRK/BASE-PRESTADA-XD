...
type GroupParticipantEntry = string | baileys.GroupParticipant | { id?: string; jid?: string }
...
const promoteOwnerIfNeeded = async (
  groupJid: string,
  participant: string,
  ...
  if (!bot || !getBotOwnerIdentityJids(bot).some((ownerJid) => sameUser(participant, ownerJid) || participant === ownerJid)) return

  const botParticipant = groupMetadata.participants.find(
    (item) => sameUser(item.id, (wss.user as any)?.id) || sameUser(item.id, (wss.user as any)?.lid),
  )
  const ownerParticipant = groupMetadata.participants.find((item) => sameUser(item.id, participant))

  if (!botParticipant?.admin || ownerParticipant?.admin) return
...
const getParticipantJid = (participant: GroupParticipantEntry): string => {
  if (typeof participant === "string") return participant
  return participant?.id || (participant as any)?.jid || ""
}

const LOG_MESSAGES = /^(1|true|yes)$/i.test(process.env.ZETA_LOG_MESSAGES || "")

const isSilencedByHigherRank = async (
  currentBotType: string,
  parentBotJid: string | undefined,
  groupMetadata: baileys.GroupMetadata,
): Promise<boolean> => {
  const higherRanks = currentBotType === "main" ? [] : currentBotType === "premium" ? ["main"] : ["main", "premium"]
  if (higherRanks.length === 0) return false
  ...
  if (higherBotIdentities.length === 0) return false
...
const getScopeForSocket = async (wss: types.WASocket, groupJid: string): Promise<{ scopedGroupJid: string; bot: types.BotDocument | null }> => {
  const lid = jidNormalizedUser((wss.user as any)?.lid || "")
  const pn = jidNormalizedUser((wss.user as any)?.id || "")
...
export const groupParticipantsUpdate = async (update: GroupParticipantsUpdate, wss: types.WASocket) => {
  try {
    ...
    if (!group || !groupMetadata) return
    if (group.mute_all_enabled) return
    ...
    for (const participant of participants) {
      ...
      if (eventCache.has(eventKey)) continue

      eventCache.add(eventKey)
      setTimeout(() => eventCache.delete(eventKey), 3000)

      if (update.action === "add") {
        if (group.autoadmin_enabled) await promoteOwnerIfNeeded(update.id, participant, groupMetadata, wss, scope.bot)
        if (silencedByHigherRank) continue
        if (!group.welcomes_enabled) continue
        if (participant === jidNormalizedUser(wss.user.lid) || participant === jidNormalizedUser(wss.user.id)) continue

        await sendWelcomeMessage(update.id, participant, group, groupMetadata, wss, scope.bot)
      } else if (update.action === "remove") {
        if (silencedByHigherRank) continue
        if (!group.farewells_enabled) continue
        if (participant === jidNormalizedUser(wss.user.lid) || participant === jidNormalizedUser(wss.user.id)) continue

        await sendFarewellMessage(update.id, participant, group, groupMetadata, wss, scope.bot)
      } else if (update.action === "promote") {
        if (silencedByHigherRank) continue
        if (!group.alerts_enabled) continue
...
const sendWelcomeMessage = async (
  ...
  try {
    ...
    const welcomeContent = await mergeCaptionWithMenuMedia("welcome", welcomeMessage, welcomeBot as any)

    await wss.sendMessage(
      groupJid,
      {
        ...(welcomeContent as any),
...
const sendFarewellMessage = async (
  ...
  try {
    ...
    const farewellContent = await mergeCaptionWithMenuMedia("welcome", farewellMessage, farewellBot as any)

    await wss.sendMessage(
      groupJid,
      {
        ...(farewellContent as any),
        mentions: [participant],
      } as any,
    )
...
export const handleWelcomeEvents = async (message: baileys.WAMessage, wss: types.WASocket) => {
  try {
    ...
    if (isWelcomeEvent && !group.welcomes_enabled) return
    if (isFarewellEvent && !group.farewells_enabled) return
