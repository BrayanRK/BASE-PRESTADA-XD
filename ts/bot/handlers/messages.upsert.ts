...
import {
  getMatchedCommandPrefix,
  handlePostConnectionSetupRaw,
  isPostConnectionSetupComplete,
} from "../../libs/zeta_setup.js"
...
import {
  ...
  getBotScopeJid,
  getBotScopeJids,
  getParticipantIdentityJids,
  isAllowedTagUserAny,
  isAntiTagEnabled,
  isBannedBotUserAny,
  isBotAdminInMetadata,
  shouldDeleteForwardedTag,
  isMutedUserAny,
  isUserAdminInMetadata,
  sameIdentity,
...
type MessageUpsert = {
  messages: baileys.WAMessage[]
  type: baileys.MessageUpsertType
  requesId?: string
}
...
const PRIVATE_BOT_CATEGORIES = new Set<types.CommandCategories>(["bot", "premb"])
...
const commandCanUsePrivateByDefault = (command: types.Command): boolean => {
...
const pruneAntiSpamState = (state: AntiSpamState, now: number): AntiSpamState => {
...
const getAntiSpamKey = (groupJid: string, senderJid: string): string => `${groupJid}:${senderJid}`
...
const promoteGroupParticipant = async (
  ...
  groupJid: string,
  userJid: string,
  metadata?: baileys.GroupMetadata | null,
): Promise<boolean> => {
...
const groupMetadataCache = new Map<string, { expiresAt: number; metadata: baileys.GroupMetadata }>()

const getSafeGroupMetadata = async (wss: types.WASocket, jid: string, force = false): Promise<baileys.GroupMetadata | null> => {
  if (!/@g\.us$/i.test(jid || "")) return null
  const cached = groupMetadataCache.get(jid)
  if (!force && cached && cached.expiresAt > Date.now()) return cached.metadata
  ...
  try {
    ...
    if (LOG_MESSAGES) console.error("[GroupMetadata] Error:", error)
...
const containsBlockedLink = (text: string): boolean => {
  return BLOCKED_LINK_REGEX.test(String(text || "").trim())
}

const AI_COMMAND_NAME = "bot"

const normalizeAiTriggerText = (value?: string | null): string =>
  String(value || "")
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
    .toLowerCase()
...
const cleanAiAlias = (value?: string | null): string =>
  String(value || "")
    .replace(/\s+/g, " ")
    .trim()

const uniqueBotNameAliases = (bot?: Partial<types.BotDocument> | null, mctx?: types.MessageContext): string[] => {
  const values = [
    bot?.name,
    bot?.username,
    mctx?.me?.name,
...
const extractBotNamePrompt = (
  ...
  for (const alias of uniqueBotNameAliases(bot, mctx).sort((a, b) => b.length - a.length)) {
    ...
    if (bodyLower === aliasLower) return ""
    if (bodyLower.startsWith(`${aliasLower} `)) return body.slice(alias.length).trim()
    if (firstTokenKey === aliasKey) return body.slice(firstToken.length).trim()
  }

  return null
}

const getBotMentionJids = (mctx: types.MessageContext, wss: types.WASocket): string[] => {
  return Array.from(new Set([mctx.me.jids.lid, mctx.me.jids.pn, wss.user?.id].filter(Boolean)))
}

const isBotMentionedJid = (jid: string, botJids: string[]): boolean => {
  return botJids.some((botJid) => sameUser(jid, botJid))
}

const extractLeadingMention = (text: string): { number: string; rest: string } | null => {
...
const removeExtraBotMentions = (text: string, botJids: string[]): string => {
...
const extractMentionAiPrompt = (text: string, mctx: types.MessageContext, wss: types.WASocket): string | null => {
  if (!mctx.is_group || mctx.message.from_me) return null

  const leadingMention = extractLeadingMention(text)
  if (!leadingMention) return null
  ...
  if (!leadingIsBot) return null

  return removeExtraBotMentions(leadingMention.rest, botJids)
}

export const messagesUpsert = async (upsert: MessageUpsert, wss: types.WASocket, instance?: any) => {
  try {
    ...
    for (const message of upsert.messages) {
      ...
      if (mctx.is_group && mctx.sender?.jid) {
        ...
        for (const [botKey, runtimeBot] of Bot.bots) {
          if (botKey === ownerBotJid) continue
          const liveUser = (runtimeBot.wss as any)?.user
          const candidates = [liveUser?.id, liveUser?.lid, runtimeBot.bot_jid].filter(Boolean) as string[]
        ...
        if (!isSiblingBotSender) {
          ...
          const myOwnJids = [wss.user?.id, (wss.user as any)?.lid].filter(Boolean).map((j) => String(j).toLowerCase())
          for (const registered of registeredBots) {
            const candidates = [registered.bot_jid].filter(Boolean) as string[]
            if (candidates.some((c) => myOwnJids.includes(c.toLowerCase()))) continue
            if (candidates.some((c) => sameUser(mctx.sender.jid, c) || mctx.sender.jid.toLowerCase() === c.toLowerCase())) {
              isSiblingBotSender = true
              break
            }
        ...
        if (isSiblingBotSender) {
          if (LOG_MESSAGES) console.log(`[SiblingGuard] Ignorando mensaje de otro bot propio: ${mctx.sender.jid}`)
          continue
        }
      }
      ...
      let runtimeBotCache: {
        botLookupJid: string
        rawBot: types.BotDocument | null
        bot: types.BotDocument | null
      } | null = null

      const getRuntimeBot = async () => {
        if (runtimeBotCache) return runtimeBotCache
      ...
      if (mctx.is_group) {
        ...
        if (earlyGroup?.mute_all_enabled) {
          ...
          const earlySameBotActor =
            mctx.message.from_me ||
          ...
          const earlyOfficialBootstrapOwner =
            (earlyBot?.bot_type === "main" || earlyBot?.bot_type === "premium") &&
            !ownerIsConfigured(earlyBotOwnerJid) &&
            (earlySameBotActor || isOwner(mctx.sender.jid))
          const earlyCanDisable =
            earlyCommand?.name === "muteall" &&
        ...
        if (guardBotJid && !senderIsBot) {
          ...
          if (guardGroup?.antispam_enabled && !userIsAdmin) {
            ...
            if (hasSpam) {
              ...
              if (LOG_MESSAGES) console.log(`[AntiSpam] Spam detectado: ${mctx.sender.jid}`)
              ...
              for (const method of kickMethods) {
                try {
                  ...
                  if (LOG_MESSAGES) console.log(`[AntiSpam] Método de expulsión falló:`, error?.message || error)
                  continue
                }
              }
          ...
          if (antiTagEnabled) {
            ...
            if (
              ...
              try {
                await mctx.delete()
                if (LOG_MESSAGES) console.log(`[AntiTag] Tag reenviado borrado: ${mctx.sender.jid}`)
              } catch (error: any) {
                if (LOG_MESSAGES) console.log(`[AntiTag] No pude borrar:`, error?.message || error)
              }
              continue
            }
          }
          ...
          if (await isMutedUserAny(guardBotJid, mctx.chat.jid, senderCheckJids)) {
            try {
              await mctx.delete()
              if (LOG_MESSAGES) console.log(`[MuteUser] Mensaje borrado: ${mctx.sender.jid}`)
            } catch (error: any) {
              if (LOG_MESSAGES) console.log(`[MuteUser] No pude borrar:`, error?.message || error)
            }
            continue
          }
        }
...
◈ Error 》 ${error?.message || error}`)
                ...
                if (!userIsAdmin && botIsAdmin) {
                  ...
                  for (const method of kickMethods) {
                    try {
                      ...
                      if (LOG_MESSAGES) console.log(`[Antilinks] Método de expulsión falló:`, error?.message || error)
                      continue
                    }
                  }
      ...
      if (usedPrefix || mentionAiPrompt !== null) {
        if (mctx.is_group) {
          ...
          const higherRanks: string[] =
            currentBotType === "main" ? [] : currentBotType === "premium" ? ["main"] : ["main", "premium"]
          ...
          if (higherRanks.length > 0) {
            try {
              ...
              if (groupMetadata?.participants?.length) {
                ...
                const higherBotIdentities: string[] = []
                for (const [, runtimeBot] of Bot.bots) {
                  if (!higherRanks.includes(runtimeBot.bot_type)) continue
                  const liveUser = (runtimeBot.wss as any)?.user
                  if (liveUser?.id) higherBotIdentities.push(liveUser.id)
                  if (liveUser?.lid) higherBotIdentities.push(liveUser.lid)
                  if (runtimeBot.bot_jid) higherBotIdentities.push(runtimeBot.bot_jid)
                }

                const registeredHigherBots = await database.Bots.listByType(higherRanks).catch(() => [])
                for (const registered of registeredHigherBots) {
                  if (registered.bot_jid) higherBotIdentities.push(registered.bot_jid)
                }
                ...
                while (parentJid && hops < 4) {
                  ...
                  if (higherRanks.includes(parentDoc.bot_type)) {
                    if (parentDoc.bot_jid) higherBotIdentities.push(parentDoc.bot_jid)
                  }
        ...
        if (commandUsed) {
          if (LOG_MESSAGES) console.log(`[Commands] Ejecutando: ${commandUsed.name}`)
          ...
          let groupMetadata: baileys.GroupMetadata | null = null
          if (mctx.is_group) {
            if (!group) {
          ...
          const sameBotActor =
            ...
            sameIdentity(mctx.sender.jid, mctx.me.jids.lid) ||
            sameIdentity(mctx.sender.jid, mctx.me.jids.pn)
          const officialBootstrapOwner =
            (bot?.bot_type === "main" || bot?.bot_type === "premium") &&
            !ownerIsConfigured(botOwnerJid) &&
            (sameBotActor || isOwner(mctx.sender.jid))
          const userIsPrimaryBotOwner =
            userIsOwner ||
          ...
          if (mctx.is_group && group?.autoadmin_enabled && userIsPrimaryBotOwner && botIsAdmin && !userIsAdmin) {
            ...
            if (promoted) {
              ...
              if (LOG_MESSAGES) console.log(`[AutoAdmin] Owner promovido/verificado: ${mctx.sender.jid}`)
            }
          }
          ...
          if (!userIsOwner && !userIsBotOwner && currentBotScopeJid) {
            ...
            if (blockedByBanUser) {
              if (LOG_MESSAGES) console.log(`[BanUser] Comando ignorado: ${mctx.sender.jid}`)
              continue
            }
          }
...
◈ Estado › usa este comando en un grupo`,
          ...
          if (commandUsed.flags.includes("only.private") && !mctx.is_private) {
            await mctx.reply(
              `*｢✧｣* El comando *${usedPrefix + commandName}* solo puede ser utilizado en el chat privado del bot.`,
            )
            continue
          }
...
◈ Requiere › premium u oficial`)
          ...
          try {
            const commandExecuteContext: types.CommandExecuteContext = {
              mctx: mctx,
              commandName: commandName,
              usedPrefix,
              args: args,
              group: group || ({} as types.GroupDocument),
              user: user,
              bot: bot || ({
              ...
              groupMetadata: groupMetadata || ({} as baileys.GroupMetadata),
            ...
            if (LOG_MESSAGES) console.log(`[Commands] ✅ ${commandUsed.name}`)
          } catch (error) {
