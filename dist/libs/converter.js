...
cleanTmpFiles();
const runFfmpeg = async (buffer, beforeInputArgs = [], afterInputArgs = [], ext = "bin", ext2 = "bin") => {
    assertBuffer(buffer);
...
export const ffmpeg = async (buffer, args = [], ext = "bin", ext2 = "bin") => {
    return runFfmpeg(buffer, [], args, ext, ext2);
};
export const ffmpegWithInputArgs = async (buffer, inputArgs = [], outputArgs = [], ext = "bin", ext2 = "bin") => {
...
export const mimeToExt = (mime) => {
    ...
    if (clean.includes("png"))
        return "png";
    if (clean.includes("jpg") || clean.includes("jpeg"))
        return "jpg";
    if (clean.includes("gif"))
        return "gif";
    ...
    if (clean.includes("wav"))
        return "wav";
    if (clean.includes("m4a"))
        return "m4a";
    if (clean.includes("webm"))
        return "webm";
    return "bin";
};
export const toPTT = async (buffer, ext = "mp4") => {
    const out = await ffmpeg(buffer, [
        "-vn",
        "-c:a",
        "libopus",
        "-b:a",
