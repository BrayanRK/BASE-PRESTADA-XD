...
const isShopContext = (value) => {
    return Boolean(value && typeof value === "object" && "mctx" in value && "bot" in value);
};
export const getShopBotJid = (bot) => {
    if (typeof bot === "string")
        return normalizeStorageJid(bot) || "default@lid";
    return getEffectiveBotJid(bot || undefined) || "default@lid";
};
export const getShopScopeKey = (ctxOrBot, groupJid) => {
...
export const getShopGroupJid = (ctxOrGroup) => {
...
export const normalizeNumber = (value) => String(value || "")
    ...
    .split("@")[0]
    .replace(/\D/g, "");
export const toUserJid = (number) => `${normalizeNumber(number)}@s.whatsapp.net`;
...
export const shopHeader = (title, lines = []) => {
    let text = `「◈」 ${title}\n`;
    for (const line of lines)
        text += `◈ ${line}\n`;
    return text.trimEnd();
};
export const ensureShopTable = async () => {
    if (tableReady)
        return tableReady;
    tableReady = new Promise((resolve) => {
        try {
            ...
            db.run(FEATURE_TABLE_SQL, (err) => {
                if (err)
                    console.error("[Shop] Error creando tabla:", err);
...
export const isShopEnabled = async (bot, groupJid) => {
    await ensureShopTable();
    const botJid = typeof bot === "string" && groupJid ? bot : getEffectiveBotJid(bot);
    const realGroupJid = groupJid || String(bot || "");
    return new Promise((resolve) => {
        try {
            const db = getConnection();
            db.get("SELECT enabled FROM group_shop_settings_v1 WHERE bot_jid = ? AND group_jid = ?", [botJid, realGroupJid], (err, row) => {
                if (err) {
...
export const setShopEnabled = async (bot, groupJid, enabled, updatedBy) => {
    await ensureShopTable();
    const botJid = typeof bot === "string" ? bot : getEffectiveBotJid(bot);
    return new Promise((resolve) => {
        try {
            const db = getConnection();
            db.run(`INSERT INTO group_shop_settings_v1 (bot_jid, group_jid, enabled, updated_by, updated_at)
         VALUES (?, ?, ?, ?, CURRENT_TIMESTAMP)
         ON CONFLICT(bot_jid, group_jid) DO UPDATE SET
           enabled = excluded.enabled,
           updated_by = excluded.updated_by,
           updated_at = CURRENT_TIMESTAMP`, [botJid, groupJid, enabled ? 1 : 0, updatedBy], (err) => {
                if (err) {
...
export const guardShopCommand = async (_wss, ctx, options = {}) => {
...
export const loadShopDb = () => readJsonFile(VENTAS_PATH, {});
export const saveShopDb = (db) => writeJsonFile(VENTAS_PATH, db);
export const getShopItemByCommand = (command) => {
...
export const getShopItem = (ctx, item) => {
    const db = loadShopDb();
...
export const listShopStoredItems = (ctx) => {
    return SHOP_ITEMS.map((item) => ({ item, data: getShopItem(ctx, item) }));
};
export const setShopItem = (ctx, item, data) => {
    const db = loadShopDb();
    const scopedKey = getShopScopeKey(ctx);
    if (!db[scopedKey])
        db[scopedKey] = {};
...
export const deleteShopItem = (ctx, itemKey) => {
    const db = loadShopDb();
    const scopedKey = getShopScopeKey(ctx);
    const legacyKey = getShopGroupJid(ctx);
    if (itemKey === "all") {
    ...
    if (!item)
        return false;
    if (db[scopedKey])
        delete db[scopedKey][item.key];
    if (db[legacyKey])
        delete db[legacyKey][item.key];
    return saveShopDb(db);
};
export const getQuotedOrDirectImageBuffer = async (ctx) => {
    const quotedMime = String(ctx.mctx.quoted?.message?.mimetype || "");
    if (/^image\//i.test(quotedMime) && ctx.mctx.quoted?.download) {
        const buffer = (await ctx.mctx.quoted.download().buffer().catch(() => null));
        if (buffer?.length)
            return buffer;
    }
    const directMime = String(ctx.mctx.message.mimetype || "");
    if (/^image\//i.test(directMime) && ctx.mctx.download) {
        const buffer = (await ctx.mctx.download().buffer().catch(() => null));
        if (buffer?.length)
            return buffer;
    }
    return null;
};
export const createShopSetCommand = (item) => ({
...
export const createShopViewCommand = (item) => ({
    name: item.command,
    alias: item.aliases || [],
    ...
    requires: [],
    execute: async (wss, ctx) => {
        if (!(await guardShopCommand(wss, ctx, { requireEnabled: true, requireOrganizer: false })))
            return;
        ...
        if (data.imagen) {
            try {
                ...
                await wss.sendMessage(ctx.mctx.chat.jid, { image: buffer, caption: data.texto || item.fallbackCaption }, { quoted: ctx.mctx.message.original });
                return;
            }
            catch (error) {
...
export const shopMenu = async (ctx) => {
...
const readRawInvoiceDb = () => readJsonFile(FACTURAS_PATH, { facturas: [], scopes: {} });
export const loadInvoiceDb = (ctx) => {
    const raw = readRawInvoiceDb();
    if (!ctx)
        return { facturas: Array.isArray(raw.facturas) ? raw.facturas : [] };
    const scopedKey = getShopScopeKey(ctx);
    const scoped = Array.isArray(raw.scopes?.[scopedKey]) ? raw.scopes[scopedKey] : [];
    return { facturas: scoped };
};
export const saveInvoiceDb = (db, ctx) => {
    if (!ctx)
        return writeJsonFile(FACTURAS_PATH, db);
    const raw = readRawInvoiceDb();
    const scopedKey = getShopScopeKey(ctx);
    raw.scopes = raw.scopes || {};
    raw.scopes[scopedKey] = Array.isArray(db.facturas) ? db.facturas : [];
    if (!Array.isArray(raw.facturas))
        raw.facturas = [];
    return writeJsonFile(FACTURAS_PATH, raw);
};
export const invoiceMatches = (invoice, numeroCliente, servicio) => {
...
export const buildInvoiceCaption = (invoice, title = "Factura generada") => {
...
export const getInvoiceStatus = (invoice) => {
    if (Date.now() > Number(invoice.fechaProximoPago || 0))
        return "no pagado";
