...
const fetchFallbackRate = async (fromCode, toCode) => {
    ...
    try {
        ...
        if (!res.ok)
            throw new CurrencyError(`HTTP ${res.status}`, "No se pudo obtener la tasa de cambio en este momento.");
...
export const getRate = async (fromCode, toCode) => {
    if (fromCode === toCode)
        return 1;
    try {
        ...
        console.error("[currency] @tamtamchik/exchanger falló, usando respaldo:", primaryError instanceof Error ? primaryError.message : primaryError);
        try {
            return await fetchFallbackRate(fromCode, toCode);
        }
        catch (fallbackError) {
            if (fallbackError instanceof CurrencyError)
                throw fallbackError;
