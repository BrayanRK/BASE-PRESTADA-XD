...
`;
let tableReady = null;
export const ensureFreeFireTable = async () => {
    if (tableReady)
        return tableReady;
    tableReady = new Promise((resolve) => {
        try {
            ...
            db.run(FEATURE_TABLE_SQL, (err) => {
                if (err)
                    console.error("[FreeFire] Error creando tabla:", err);
...
export const isFreeFireEnabled = async (bot, groupJid) => {
    ...
    return new Promise((resolve) => {
        try {
            const db = getConnection();
            db.get("SELECT enabled FROM group_freefire_settings_v2 WHERE bot_jid = ? AND group_jid = ?", [botJid, realGroupJid], (err, row) => {
                if (err) {
...
export const setFreeFireEnabled = async (bot, groupJid, enabled, updatedBy) => {
    await ensureFreeFireTable();
    const botJid = typeof bot === "string" ? bot : getEffectiveBotJid(bot);
    return new Promise((resolve) => {
        try {
            const db = getConnection();
            db.run(`INSERT INTO group_freefire_settings_v2 (bot_jid, group_jid, enabled, updated_by, updated_at)
...
export const normalizeJidNumber = (jid) => String(jid || "").split("@")[0].split(":")[0].replace(/\D/g, "");
export const mentionTag = (jid) => `@${normalizeJidNumber(jid) || jid.split("@")[0]}`;
export const isFreeFireOrganizer = (ctx) => {
    return Boolean(ctx.userIsAdmin || ctx.userIsOwner || ctx.userIsBotOwner || ctx.mctx.message.from_me);
};
export const freeFireHeader = (title, lines = []) => {
    let text = `「◈」 🔥 ${title}\n`;
    for (const line of lines)
        text += `◈ ${line}\n`;
    return text.trimEnd();
};
export const sendFreeFireDisabled = async (ctx) => {
...
export const guardFreeFireCommand = async (_wss, ctx, options = {}) => {
...
export const getGroupParticipants = async (wss, ctx) => {
    let metadata = ctx.groupMetadata;
    if (!metadata?.participants?.length)
        metadata = await wss.groupMetadata(ctx.mctx.chat.jid);
...
export const to12Hour = (hour, minute) => {
    const normalizedHour = ((hour % 24) + 24) % 24;
    const suffix = normalizedHour >= 12 ? "pm" : "am";
    const displayHour = normalizedHour % 12 || 12;
    return `${displayHour}:${String(minute).padStart(2, "0")}${suffix}`;
};
export const renderSchedule = (rawTime) => {
    const parsed = parseTimeInput(rawTime || DEFAULT_BASE_TIME);
    if (!parsed)
        return "╎ Formato de hora inválido. Ejemplo: 8:00pm";
    const zones = [
...
export const buildVersusText = (config, groupName, rawTime, participants) => {
...
export const createFreeFireVersusCommand = (config) => {
    ...
    return {
        ...
        execute: async (wss, ctx) => {
            ...
            if (!rawTime || !parseTimeInput(rawTime)) {
                await ctx.mctx.reply(`${freeFireHeader(config.label, ["Uso 》 " + ctx.usedPrefix + config.command + " 8:00pm"])}\n\n╎ Formatos válidos: *8pm*, *8:30pm*, *20:30*`);
                return;
            }
            ...
            if (participants.length < required) {
                await ctx.mctx.reply(`${freeFireHeader("Faltan jugadores", [
                    `Modalidad 》 ${config.label}`,
                    `Necesarios 》 ${required}`,
...
export const freeFireMenu = async (ctx) => {
