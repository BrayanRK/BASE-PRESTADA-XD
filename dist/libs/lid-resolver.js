...
const isRawUserJid = (value) => /@(lid|s\.whatsapp\.net)$/i.test(stripDevice(value));
const isLidJid = (value) => /@lid$/i.test(stripDevice(value));
const isPhoneJid = (value) => /@s\.whatsapp\.net$/i.test(stripDevice(value));
export const jidDigits = (value) => stripDevice(value).split("@")[0].replace(/[^0-9]/g, "");
export const isValidPhoneNumber = (digits) => /^\d{8,15}$/.test(String(digits || ""));
...
const phoneDigitsFromValue = (value) => {
    const raw = stripDevice(value);
    if (!raw)
        return "";
    if (isPhoneJid(raw))
        return jidDigits(raw);
    if (!raw.includes("@")) {
        const digits = jidDigits(raw);
        return isValidPhoneNumber(digits) ? digits : "";
    }
...
const jidValuesFromParticipant = (participant) => {
    const values = [
        participant?.id,
        participant?.jid,
        participant?.lid,
...
const participantMatchesTarget = (participant, target) => {
    ...
    if (targetJid && values.includes(targetJid))
        return true;
...
const emptyResolution = (input, note = "No se encontró información suficiente.") => ({
    ok: false,
    exists: false,
    input,
...
export const isTargetInGroupMetadata = (target, metadata) => {
    const participants = Array.isArray(metadata?.participants) ? metadata?.participants || [] : [];
    if (!participants.length)
        return false;
    return participants.some((item) => participantMatchesTarget(item, target));
};
const resolveFromGroupMetadata = (target, metadata) => {
    const participants = Array.isArray(metadata?.participants) ? metadata?.participants || [] : [];
    if (!participants.length)
        return null;
    const participant = participants.find((item) => participantMatchesTarget(item, target));
    if (!participant)
        return null;
...
const resolveWithOnWhatsApp = async (wss, number) => {
    if (!isValidPhoneNumber(number))
        return null;
    try {
        const result = (await wss.onWhatsApp(number));
...
export const resolveUserLid = async (wss, target, options = {}) => {
    const input = clean(target);
    if (!input)
        return emptyResolution(input);
    const selfJids = [options.mctx?.me?.jids?.lid, options.mctx?.me?.jids?.pn, wss.user?.id, wss.user?.lid]
...
export const formatLidResolution = (result) => {
    const lines = [
        `「✧」 LID`,
