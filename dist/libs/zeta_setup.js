...
import { DEFAULT_COMMAND_PREFIXES, getUniversalConfig, isRuntimeSetupComplete, normalizeOwnerJid, normalizePrefixes, parseOptionalUrl, updateUniversalConfig, } from "./zeta_cf.js";
...
const getChatCacheKeys = (message, wss, config = getUniversalConfig()) => {
    const keys = new Set();
...
const getPromptTarget = (wss, config = getUniversalConfig()) => config.setup?.lockedChatJid || getBotPnJid(wss) || getBotLid(wss);
const isOwnBotChat = (message, wss, config = getUniversalConfig()) => {
    const chatJid = getRawChatJid(message);
...
const isOwnSetupNoticeRaw = (message) => {
    ...
    return (text.startsWith("*Configuración de ") ||
...
const getCurrentStep = (config = getUniversalConfig()) => {
    for (let index = 0; index < assetSteps.length; index++) {
        const step = assetSteps[index];
        if (!config.setup?.assets?.[step.key]?.path)
            return { type: "asset", index, ...step };
    }
    for (const step of textSteps) {
        if (!config.setup?.textCompleted?.[step.key])
...
const sendSafe = async (wss, jid, text, quoted) => {
    if (!jid)
        return;
    try {
...
const sendWarningOnce = async (wss, jid, key, text, quoted) => {
    const now = Date.now();
...
const sendCurrentPrompt = async (wss, force = false) => {
    ...
    if (!force && lastPromptKey === promptKey)
        return;
...
const syncSetupAssetsToBotRecord = async (wss, config) => {
    try {
        const database = await import("../database/database.js");
        const jids = new Set();
        if (wss.user?.id)
            jids.add(safeNormalizeJid(wss.user.id));
        if (wss.user?.lid)
            jids.add(getBotLid(wss));
...
const completeSetup = async (wss, chatJid, prefixes) => {
...
export const ensurePostConnectionSetup = async (wss) => {
    if (isRuntimeSetupComplete())
        return;
    await sendCurrentPrompt(wss, true);
};
export const isPostConnectionSetupComplete = () => isRuntimeSetupComplete();
export const getMatchedCommandPrefix = (text, bot) => {
    if (!isRuntimeSetupComplete())
        return "";
    const config = getUniversalConfig();
    const botPrefixes = normalizePrefixes(bot?.prefixes || "");
...
export const handlePostConnectionSetupRaw = async (message, wss) => {
    const config = getUniversalConfig();
    if (config.setup?.completed)
        return false;
    if (!message?.message)
    ...
    if (isOwnSetupNoticeRaw(message))
        return true;
    if (!isOwnBotChat(message, wss, config))
        return true;
    const text = getTextFromContent(message.message);
    ...
    try {
        ...
        if (step.type === "text") {
            ...
            if (!parsed.ok) {
                await sendWarningOnce(wss, chatJid, `link:${step.key}`, `「◈」 CONFIGURACIÓN
◈ Enlace inválido
◈ Ejemplo › https://instagram.com/usuario
◈ Omitir › envía 0`, message);
                return true;
            }
            const socialUpdate = {};
            const configUpdate = {
            ...
            if (step.key === "channelUrl")
                configUpdate.channelUrl = parsed.value;
            if (step.key === "facebookUrl")
                socialUpdate.facebook = parsed.value;
            if (step.key === "instagramUrl")
                socialUpdate.instagram = parsed.value;
            if (step.key === "tiktokUrl")
                socialUpdate.tiktok = parsed.value;
            if (step.key === "telegramUrl")
                socialUpdate.telegram = parsed.value;
            if (Object.keys(socialUpdate).length)
                configUpdate.socialLinks = socialUpdate;
...
export const handlePostConnectionSetupMessage = async (mctx, wss) => {
    if (getUniversalConfig().setup?.completed)
        return false;
    const original = mctx.message.original;
    if (original?.message)
        return handlePostConnectionSetupRaw(original, wss);
    return true;
};
