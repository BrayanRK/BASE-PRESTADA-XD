import { freeFireHeader, mentionTag, parseTimeInput, shuffle } from "./freefire.js";
const sessions = new Map();
const INSCRIPCION_TTL_MS = 10 * 60 * 1000;
const MAX_TEAM_SIZE = 24;
export const parseVsMode = (raw) => {
    ...
    if (!Number.isFinite(a) || !Number.isFinite(b) || a !== b)
        return null;
    if (a < 1 || a > MAX_TEAM_SIZE)
        return null;
    return { raw: `${a}vs${a}`, teamSize: a, label: `${a} vs ${a}` };
};
const formatTime = (rawTime) => parseTimeInput(rawTime)?.normalized || rawTime;
const renderInscritos = (session) => {
    if (!session.participants.size)
        return "╎ sin inscritos aún";
    return Array.from(session.participants.keys())
        .map((jid, index) => `╎ ${index + 1}. ${mentionTag(jid)}`)
        .join("\n");
};
const buildInscripcionText = (session) => {
...
const clearSessionTimer = (groupJid) => {
...
const expireSession = async (wss, groupJid) => {
    const session = sessions.get(groupJid);
    if (!session || session.finished)
        return;
    session.finished = true;
    clearSessionTimer(groupJid);
    const text = freeFireHeader("Inscripción cancelada", [
        `Modalidad 》 ${session.mode.label}`,
...
const finalizeSession = async (wss, session) => {
    session.finished = true;
...
const addParticipant = async (wss, session, jid) => {
    if (session.finished)
        return false;
    ...
    await wss
        ...
        .catch(() => { });
    if (session.participants.size >= session.required)
        await finalizeSession(wss, session);
    return true;
};
export const hasActiveInscripcion = (groupJid) => sessions.has(groupJid);
export const startInscripcion = async (wss, ctx, mode, rawTime) => {
    const groupJid = ctx.mctx.chat.jid;
    if (sessions.has(groupJid)) {
        const active = sessions.get(groupJid);
        await ctx.mctx.reply(freeFireHeader("Ya hay una inscripción activa", [`Modalidad 》 ${active.mode.label}`]) +
    ...
    const placeholder = freeFireHeader("Inscripción Free Fire", [
    ...
    const sent = await wss.sendMessage(groupJid, { text: placeholder, mentions: [organizerJid] }, { quoted: ctx.mctx.message.original });
...
export const handleInscripcionReply = async (wss, mctx) => {
    if (!sessions.size)
        return;
    ...
    if (!quotedId || quotedId !== session.messageId)
        return;
