...
import { getRuntimeSocialLinks, parseOptionalUrl, updateUniversalConfig, } from "../../../libs/zeta_cf.js";
...
const validateLink = (social, rawValue) => {
    const raw = cleanText(rawValue);
    if (clearWords.has(normalizeKey(raw)))
        return { ok: true, value: "", cleared: true };
    if (!raw)
        return { ok: false, value: "", error: "falta el enlace" };
    if (raw.length > 500)
        return { ok: false, value: "", error: "el enlace es demasiado largo" };
    if (/javascript:|data:|file:/i.test(raw))
        return { ok: false, value: "", error: "protocolo no permitido" };
    const parsed = parseOptionalUrl(raw);
    if (!parsed.ok || !parsed.value)
        return { ok: false, value: "", error: "enlace inválido" };
    if (!hostMatches(parsed.value, social.domains)) {
        return { ok: false, value: "", error: `ese enlace no pertenece a ${social.label}` };
    }
    return { ok: true, value: parsed.value };
};
const unique = (values) => Array.from(new Set(values.map((value) => cleanText(value).toLowerCase()).filter(Boolean)));
const getBotStorageKeys = (bot, mctx) => unique([
...
const updateBotFieldEverywhere = async (keys, field, value) => {
    await Promise.all(keys.map((key) => database.Bots.update(key, { $set: { [field]: value } })));
};
const setUniversalSocialLink = (links, key, value) => {
    if (key === "facebook")
        return { ...links, facebook: value };
    if (key === "instagram")
        return { ...links, instagram: value };
    if (key === "tiktok")
        return { ...links, tiktok: value };
    return { ...links, telegram: value };
};
const syncUniversalMain = (social, value, bot) => {
