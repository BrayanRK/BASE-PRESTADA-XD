...
const getFreeCodeSessionPath = (targetNumber) => {
...
export const createFreeCodeSocket = async (wss, mctx, phoneNumber, parentBot) => {
    if (!mctx.is_group) {
    ...
    if (existingActive) {
        ...
        await mctx.reply(box("SOCKET FREE", [
            `Número › ${targetNumber}`,
    ...
    ws.ev.on("bot.code", async (e) => {
        if (hasSentCredential || isConnected)
            return;
        hasSentCredential = true;
        const { key } = await wss.sendMessage(mctx.chat.jid, {
        ...
        setTimeout(async () => {
            if (!isConnected)
                await wss.sendMessage(mctx.chat.jid, { delete: key }).catch(() => null);
        }, 60_000);
    });
