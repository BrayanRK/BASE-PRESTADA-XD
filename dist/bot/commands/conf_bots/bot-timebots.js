...
const command = {
    ...
    execute: async (wss, { mctx }) => {
        ...
        const entries = Array.from(Bot.bots.entries()).sort(([, a], [, b]) => {
            const typeOrder = (t) => (t === "main" ? 0 : t === "premium" ? 1 : 2);
        ...
        const lines = [`「⏱」 Tiempo de conexión — ${entries.length} socket${entries.length !== 1 ? "s" : ""}`];
        for (const [, botData] of entries) {
            const botJid = botData.bot_jid || "";
            const botNumber = botJid.split("@")[0].split(":")[0] || "?";
            const name = await wss.getName(botJid).catch(() => "") || botNumber;
            const type = getBotType(botData.bot_type || "free");
            let uptimeStr;
