...
const command = {
    ...
    execute: async (wss, { mctx, group, bot }) => {
        ...
        if (timeDifference < miningInterval) {
            ...
            await mctx.reply(`*｢✧｣* Debes esperar *${libs.formatDuration(remainingTime)}* para volver a cazar pokémon en este grupo.`);
            return;
        }
        ...
        `);
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?)
        `);
                stmt2.run(mctx.sender.jid, scopedGroupJid, selectedPokemon.id, selectedPokemon.name, JSON.stringify(selectedPokemon.types), JSON.stringify(selectedPokemon.base_stat), selectedPokemon.sprite);
