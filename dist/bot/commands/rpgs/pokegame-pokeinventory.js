    ...
    execute: async (wss, { mctx, group, args, bot }) => {
        ...
        return new Promise((resolve) => {
            try {
                ...
                conn.all(`SELECT * FROM user_pokemon WHERE user_jid = ? AND group_jid = ? ORDER BY pokemon_name ASC`, [mentioned, scopedGroupJid], async (err, rows) => {
                    if (err) {
                    ...
                    if (!rows || rows.length === 0) {
                        const message = mentioned === mctx.sender.jid
