...
const command = {
    ...
    execute: async (_, { mctx, group, bot, usedPrefix }) => {
        ...
        const timeDifference = now - (groupUser.last_crime_ago || 0);
        ...
        try {
            ...
            if (won) {
                conn.run(`UPDATE group_users SET money = money + ?, last_crime_ago = ? WHERE group_jid = ? AND user_jid = ?`, [reward, now, scopedGroupJid, mctx.sender.jid]);
                await mctx.reply(`✦ ${libs.pickRandom(successMessages)} *${formatMoney(reward, currency)}*.`);
                return;
            }
            conn.run(`UPDATE group_users SET money = money - ?, last_crime_ago = ? WHERE group_jid = ? AND user_jid = ?`, [penalty, now, scopedGroupJid, mctx.sender.jid]);
