...
const sessionKey = (chatJid, userJid) => `${chatJid}:${userJid}`;
const drawCard = () => ({ rank: RANKS[randomInt(0, RANKS.length - 1)], suit: SUITS[randomInt(0, SUITS.length - 1)] });
...
const settleGame = async (wss, mctx, key, game, currency, reason) => {
    ...
    try {
        ...
        conn.run(`UPDATE group_users SET money = money + ? WHERE group_jid = ? AND user_jid = ?`, [delta, game.scopedGroupJid, game.userJid]);
...
const command = {
    ...
    execute: async (wss, { mctx, args, group, bot, usedPrefix }) => {
        ...
        try {
            const conn = getConnection();
            conn.run(`UPDATE group_users SET money = money - ? WHERE group_jid = ? AND user_jid = ? AND money >= ?`, [amount, scopedGroupJid, mctx.sender.jid, amount]);
        }
        catch (error) {
