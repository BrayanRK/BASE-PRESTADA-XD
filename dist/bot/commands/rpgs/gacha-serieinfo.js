import { formatNumber, getDisplayName, getRuntimeGacha, sendText, usageBlock, } from "../../../libs/gacha.js";
...
const paginate = (items, inputPage = 1, pageSize = 14) => {
...
const findSourceTitle = (characters, query) => {
...
const isSameSerie = (source, query) => {
...
const formatOwnerStatus = async (wss, mctx, owner) => {
    if (!owner)
        return "Libre.";
    ...
    execute: async (wss, { mctx, args, bot, group, usedPrefix }) => {
        ...
        try {
            ...
            if (!characters.length) {
                await mctx.react("⚠️");
                await sendText(wss, mctx, `✿ *Nombre:* ‹*${query}*›\n\n` +
                    `▢ Personajes » *0*\n` +
                    `♡ Reclamados » *0/0 (0%)*\n` +
                    `▢ Lista de personajes:\n\n` +
