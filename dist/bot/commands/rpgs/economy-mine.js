...
const command = {
    ...
    execute: async (wss, { mctx, group, bot, usedPrefix }) => {
        ...
        try {
            ...
            conn.run(`UPDATE group_users SET money = money + ?, last_mining_ago = ? WHERE group_jid = ? AND user_jid = ?`, [reward, now, scopedGroupJid, mctx.sender.jid]);
            const newBalance = Math.max(0, Math.floor(Number(groupUser.money) || 0)) + reward;
            await mctx.reply(`「◈」 ${message} *${formatMoney(reward, currency)}*\n` +
