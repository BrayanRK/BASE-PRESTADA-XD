...
const formatNames = (prefix, names) => {
    return names.map((name) => `\`${prefix}${name}\``).join(" ");
};
const sectionBlock = (prefix, section) => {
...
export default {
    ...
    execute: async (wss, { mctx, usedPrefix, bot }) => {
        ...
        message += `╭─〔 ✦ CRÉDITOS 〕\n│ ${FOOTER}\n╰────────────`;
        await wss.sendMessage(mctx.chat.jid, await mergeCaptionWithMenuMedia("rpg", message, bot), {
