...
const command = {
    ...
    execute: async (_, { mctx, args, group, bot, usedPrefix, commandName }) => {
        ...
        try {
            ...
            conn.run(`UPDATE group_users SET money = money - ?, money_deposited = money_deposited + ?, updated_at = CURRENT_TIMESTAMP WHERE group_jid = ? AND user_jid = ?`, [amount, amount, scopedGroupJid, mctx.sender.jid]);
