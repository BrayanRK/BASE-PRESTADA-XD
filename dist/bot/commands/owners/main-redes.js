...
export default {
    ...
    execute: async (wss, { mctx, bot }) => {
        ...
        const channelUrl = cleanText(bot.channel_url, useRuntimeFallback ? getRuntimeChannelUrl() : "");
        const links = {
            facebook: cleanText(bot.facebook_url, useRuntimeFallback ? runtimeLinks.facebook : ""),
            instagram: cleanText(bot.instagram_url, useRuntimeFallback ? runtimeLinks.instagram : ""),
            tiktok: cleanText(bot.tiktok_url, useRuntimeFallback ? runtimeLinks.tiktok : ""),
            telegram: cleanText(bot.telegram_url, useRuntimeFallback ? runtimeLinks.telegram : ""),
        };
        const ownerName = cleanText(bot.owner_name, getRuntimeOwnerName() || "Owner");
        const ownerNumber = jidNumber(bot.owner_jid);
        const rows = [];
        if (channelUrl)
            rows.push(["Canal", channelUrl]);
        if (links.facebook)
            rows.push(["Facebook", links.facebook]);
        if (links.instagram)
            rows.push(["Instagram", links.instagram]);
        if (links.tiktok)
            rows.push(["TikTok", links.tiktok]);
        if (links.telegram)
            rows.push(["Telegram", links.telegram]);
        let text = `「♛」 Redes del Owner\n`;
        text += `│ Owner › ${ownerName}\n`;
        if (ownerNumber)
            text += `│ Contacto › @${ownerNumber}\n`;
        if (!rows.length) {
