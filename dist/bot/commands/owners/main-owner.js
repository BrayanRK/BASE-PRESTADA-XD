...
const jidNumber = (jid) => String(jid || "")
    ...
    .split("@")[0]
    .replace(/[^0-9]/g, "");
const isPhoneJid = (jid) => /@s\.whatsapp\.net$/i.test(String(jid || ""));
const isValidPhoneNumber = (digits) => /^\d{8,15}$/.test(digits);
const pickOwnerPhone = (...values) => {
    for (const value of values) {
        const raw = cleanText(value);
        const digits = jidNumber(raw);
        if (!digits || !isValidPhoneNumber(digits))
            continue;
        if (/@lid$/i.test(raw) && !isPhoneJid(raw))
            continue;
        return digits;
    }
    return "";
...
export default {
    ...
    execute: async (wss, { mctx, bot }) => {
        ...
        const ownerNumber = pickOwnerPhone(bot.owner_number, isMain ? runtimeOwnerNumber : "", isMain ? runtimeOwnerJid : "", bot.owner_jid, runtimeOwnerJid);
        const ownerName = cleanText(isMain ? runtimeOwnerName || bot.owner_name : bot.owner_name || runtimeOwnerName, "Owner");
        if (!ownerNumber) {
            await mctx.reply(`「♛」 Owner\n│ Estado › sin número válido\n╰ Configura el owner usando número real, ejemplo: *${mctx.message.text?.[0] || "."}setbotowner 573161325891*`);
            return;
        }
