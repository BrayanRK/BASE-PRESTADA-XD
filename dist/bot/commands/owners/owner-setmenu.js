...
import { buildMenuDesignHelp, isMenuPreset, loadMenuDesign, materializeEscapes, normalizeMenuCategory, parseCustomMenuDesign, renderMenuCommand, renderMenuFooter, renderMenuHeader, renderMenuSection, resetMenuDesign, saveMenuDesign, } from "../../../libs/menu-design.js";
const okBox = (lines) => ["╭─〔 ✦ SETMENU ✦ 〕", ...lines.map((line) => `│ ${line}`), "╰────────────"].join("\n");
const getRawBody = (mctx, usedPrefix, commandName, args) => {
...
const saveAndReply = async (mctx, botJid, design, okText) => {
    ...
    execute: async (wss, { mctx, args, bot, usedPrefix, commandName }) => {
        ...
        if (["diseño", "design"].includes(action)) {
            ...
            if (isMenuPreset(template.toLowerCase()) && template.toLowerCase() !== "custom") {
                design.preset = template.toLowerCase();
