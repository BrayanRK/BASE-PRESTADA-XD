...
import { loadMenuDesign, renderMenuCommand, renderMenuFooter, renderMenuHeader, renderMenuSection, resolveMenuSectionMeta, } from "../../../libs/menu-design.js";
...
const jidDigits = (jid) => String(jid ?? "").split("@")[0].replace(/\D/g, "");
const spacer = () => {
...
const commandNameCute = (prefix, command) => `${prefix}${command.name}`;
const formatUsing = (using) => {
    const text = cleanText(using);
    return text ? ` _${text}_` : "";
};
const formatUsingCute = (using) => {
    const text = cleanText(using);
    return text ? ` ${text}` : "";
};
const formatDescription = (description, currency) => {
...
const visibleForUser = (command, ctx) => {
    if (command.hidden)
        return false;
    if (command.category === "owner")
        return ctx.userIsOwner || ctx.userIsBotOwner;
    if (command.category === "moderation")
        return ctx.userIsOwner || ctx.userIsBotOwner;
    if (command.category === "bot")
        return ctx.userIsOwner || ctx.userIsBotOwner;
    if (command.category === "premb")
        return ctx.userIsMod || ctx.userIsOwner || ctx.userIsBotOwner;
    return true;
};
const getDisplayCategory = (command) => {
    if (command.category === "moderation")
        return "owner";
    if (command.category === "bot" || command.category === "premb")
        return "sockets";
    return command.category;
};
const categoryBlock = (category, commands, prefix, currency, design) => {
    ...
    const renderedCommands = commands
        .map((command) => renderMenuCommand(design, {
        icon: meta.icon,
        emoji: meta.icon,
...
export default {
    ...
    hidden: false,
    requires: [],
    execute: async (wss, { mctx, usedPrefix, bot, userIsAdmin, userIsMod, userIsOwner, userIsBotOwner, }) => {
        ...
        if (footer)
            message += footer.endsWith("\n") ? footer : `${footer}\n`;
        const content = (await mergeCaptionWithMenuMedia("main", message, bot));
        ...
        await wss.sendMessage(mctx.chat.jid, content, {
