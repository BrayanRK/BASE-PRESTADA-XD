...
const promoteOwnerNow = async (wss, groupJid, ownerJid, groupMetadata) => {
...
const command = {
    ...
    execute: async (wss, { mctx, args, group, bot, userIsPrimaryBotOwner, botIsAdmin, groupMetadata }) => {
        ...
        if (requested === null)
            return void await mctx.reply(`「✧」 AutoAdmin ${group.autoadmin_enabled ? "activo" : "desactivado"}.`);
        ...
        if (requested)
            await promoteOwnerNow(wss, mctx.chat.jid, mctx.sender.jid, groupMetadata).catch(() => false);
