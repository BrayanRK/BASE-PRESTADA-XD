...
const getTargetJid = (mctx, args) => {
    const mention = extractMentionJid(mctx);
    if (mention)
        return baileys.jidNormalizedUser(mention);
    if (args.length > 0) {
        const jid = resolveJid(args.join("").trim());
        return jid || null;
    }
    if (mctx.quoted)
        return baileys.jidNormalizedUser(mctx.quoted.sender.jid);
    return null;
};
const downloadBuffer = async (msg) => {
    try {
        const buf = await baileys.downloadMediaMessage(msg, "buffer", {});
...
export default {
    ...
    execute: async (wss, { mctx, args }) => {
        ...
        try {
            await mctx.react("⏳");
            const fetchStatus = wss.fetchStatus;
            let statusText = null;
            let statusDate = null;
            ...
            const store = wss._store ?? wss.store ?? null;
            const statusMessages = [];
            if (store && typeof store.loadMessages === "function") {
                try {
                    const msgs = await store.loadMessages(statusBroadcastJid, 50, undefined);
