...
const usage = () => "「⚠」 Escribe un texto.";
const fetchBratBuffer = async (url) => {
    if (/^data:/i.test(url)) {
        const match = url.match(/^data:([^;]+);base64,(.+)$/);
        if (!match)
            throw new Error("URL de datos inválida");
    ...
    execute: async (wss, { mctx, args, bot, user }) => {
        ...
        try {
            ...
            try {
                ...
                console.error("[brat] API principal falló, usando respaldo de emergencia (kepolu):", primaryError instanceof Error ? primaryError.message : primaryError);
