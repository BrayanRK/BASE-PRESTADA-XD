...
const normalizeKey = (raw) => raw
    ...
    .trim()
    .replace(/\s+/g, " ");
const reg = (code, flag, label, aliases) => {
    for (const alias of aliases) {
        const key = normalizeKey(alias);
        if (!key)
            continue;
        if (!COUNTRIES[key])
            COUNTRIES[key] = { code, flag, label };
        const noSpace = key.replace(/\s+/g, "");
        if (noSpace !== key && !COUNTRIES[noSpace])
            COUNTRIES[noSpace] = { code, flag, label };
    }
};
...
export default {
    ...
    execute: async (_wss, { mctx, args }) => {
        ...
        if (!fromCountry || !toCountry) {
            ...
            await mctx.reply(`「✖」 País no reconocido › ${cualNoExiste}\n✦ Usa *.listcb* para ver la lista de países disponibles.`);
            return;
        }
        try {
            await mctx.react("💱");
