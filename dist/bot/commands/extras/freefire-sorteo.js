import { freeFireHeader, getGroupParticipants, guardFreeFireCommand, parseTimeInput, renderPlayers, renderSchedule, shuffle, } from "../../../libs/freefire.js";
const command = {
    name: "ffsorteo",
    ...
    execute: async (wss, ctx) => {
        ...
        if (!rawTime || !parseTimeInput(rawTime)) {
            await ctx.mctx.reply(`${freeFireHeader("Sorteo Free Fire", [`Uso › ${ctx.usedPrefix}ffsorteo 4 8:00pm`])}\n\n╎ El primer número es la cantidad de escuadras.`);
            return;
        }
        ...
        await wss.sendMessage(ctx.mctx.chat.jid, { text, mentions: Array.from(new Set(mentions)) }, { quoted: ctx.mctx.message.original });
    },
};
