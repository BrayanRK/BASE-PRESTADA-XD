...
const statusText = (enabled) => enabled ? "activo" : "desactivado";
export default {
    name: "antitag",
    alias: [],
    ...
    execute: async (_wss, { mctx, args, bot }) => {
        ...
        if (!arg)
            return void await mctx.reply(`「✧」 Antitag ${statusText(enabled)}.`);
        if (!/^(on|off)$/i.test(arg))
            return void await mctx.reply("「✖」 Usa on/off.");
        const next = arg === "on";
        if (enabled === next)
            return void await mctx.reply(`「✓」 Antitag ya estaba ${statusText(next)}.`);
