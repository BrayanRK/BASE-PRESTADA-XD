...
import { dvyerYtMp4, dvyerMediaUrl, dvyerTikTokMp4, dvyerFacebook, dvyerInstagram, dvyerSpotify, dvyerMediafire, evogbYtMp4, evogbMediaUrl, evogbTikTok, evogbSpotifyDl, downloadTikTokRaw, downloadInstagram, downloadFacebook, downloadPinterest, } from "../../../libs/downloads.js";
export const DW_KEY = "lucasxt:dw:enabled";
const url = (text) => text.match(/https?:\/\/\S+/i)?.[0]?.replace(/[>)\].,;]+$/, "") ?? "";
const platform = (text) => {
    const u = url(text) || text;
    if (/(?:youtube\.com\/(?:watch|shorts|live)|youtu\.be\/)/i.test(u))
        return "youtube";
    if (/(?:www\.|vm\.|vt\.|m\.)?tiktok\.com\//i.test(u))
        return "tiktok";
    if (/(?:facebook\.com|fb\.watch|fb\.com)\//i.test(u))
        return "facebook";
    if (/instagram\.com\//i.test(u))
        return "instagram";
    if (/(?:pinterest\.[a-z.]+\/|pin\.it\/)/i.test(u))
        return "pinterest";
    if (/open\.spotify\.com\//i.test(u))
        return "spotify";
    if (/(?:twitter\.com|x\.com)\/[^\s]+\/status\//i.test(u))
        return "twitter";
    if (/mediafire\.com\/file\//i.test(u))
        return "mediafire";
    return null;
};
const send = async (wss, mctx, media, caption) => {
    ...
    const link = String(media.url || media.directUrl || "");
    if (!link)
        return;
    if (mime.startsWith("audio/"))
...
const sendUrl = async (wss, mctx, link, mime, caption) => {
    const opts = { quoted: mctx.message.original };
    if (mime.startsWith("audio/"))
        await wss.sendMessage(mctx.chat.jid, { audio: { url: link }, mimetype: mime, ptt: false }, opts);
    else
...
export const handleAutoDownload = async (wss, mctx, bot) => {
    const botJid = getEffectiveBotJid(bot);
    if (!botJid || mctx.message.from_me)
        return;
    ...
    try {
        if (plat === "youtube") {
            let mediaUrl = "";
            try {
                mediaUrl = dvyerMediaUrl(await dvyerYtMp4(link, "720"));
            }
            catch {
            ...
            try {
                await send(wss, mctx, toMedia(await downloadTikTokRaw(link)), "「◈」 TikTok");
            }
            catch {
                let mediaUrl = "";
                try {
                    mediaUrl = evogbMediaUrl(await evogbTikTok(link));
                }
                catch {
            ...
            try {
                await send(wss, mctx, await downloadFacebook(link), "「◈」 Facebook");
            }
            catch {
                await sendUrl(wss, mctx, dvyerMediaUrl(await dvyerFacebook(link)), "video/mp4", "「◈」 Facebook");
            }
        }
            ...
            try {
                ...
                await sendUrl(wss, mctx, dvyerMediaUrl(await dvyerInstagram(link)), "video/mp4", "「◈」 Instagram");
            }
        }
            ...
            const media = medias[0];
            if (!media)
                throw new Error("Sin media de Pinterest");
            await send(wss, mctx, media, "「◈」 Pinterest");
        }
        else if (plat === "spotify") {
            let mediaUrl = "";
            try {
                mediaUrl = dvyerMediaUrl(await dvyerSpotify(link));
            }
            catch {
            ...
            await wss.sendMessage(mctx.chat.jid, { document: { url: mediaUrl }, mimetype: "application/octet-stream", fileName: "mediafire-file" }, { quoted: mctx.message.original });
        }
        await mctx.react("✅");
    }
    catch (e) {
