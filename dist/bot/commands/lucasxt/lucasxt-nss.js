...
export const runNssLookup = async (wss, mctx, rawCurp) => {
...
export const matchStandaloneCurp = (text) => {
...
async function submitReport(curp, idempotencyKey) {
    let lastError;
    const maxAttempts = 3;
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        try {
            ...
            if (!result.response.ok)
                throw createApiError(result, "submit");
            ...
            if (isTemporaryServiceFailure)
                retryDelay = 60_000 * (2 ** (attempt - 1));
            else if (isConcurrentIdempotencyRequest)
                retryDelay = 5_000;
            else if (isInternalServerFailure)
                retryDelay = 5_000 * attempt;
            await sleep(retryDelay);
        }
    }
    throw lastError;
}
async function waitForReport(uuid) {
...
function decodePdf(value, uuid) {
    if (typeof value !== "string" || !value.trim())
        throwPdfError("El documento recibido está vacío.", "pdf_empty", uuid);
    const cleanBase64 = value.replace(/^data:application\/pdf;base64,/i, "").replace(/\s+/g, "");
    if (!isValidBase64(cleanBase64))
        throwPdfError("El documento recibido no tiene un formato válido.", "pdf_base64", uuid);
    const pdfBuffer = Buffer.from(cleanBase64, "base64");
    if (!pdfBuffer.length)
        throwPdfError("El documento recibido está vacío.", "pdf_empty", uuid);
    if (pdfBuffer.subarray(0, 4).toString("ascii") !== "%PDF")
        throwPdfError("El archivo recibido no corresponde a un PDF válido.", "pdf_signature", uuid);
    return pdfBuffer;
}
function createApiError(result, stage, uuid = null) {
...
function extractApiDetails(data, raw = "", status = 0) {
    let message = "";
    let description = "";
    if (typeof data?.detail === "string")
        message = data.detail.trim();
    else if (Array.isArray(data?.detail))
        message = data.detail.map(formatValidationDetail).join(" | ");
    ...
    if (!message)
        message = String(raw || "").trim() || "El servicio devolvió un error.";
    return { message, description, code: resolveApiErrorCode(message, status, data) };
}
function resolveApiErrorCode(message, status = 0, data = null) {
    const cleanMessage = String(message || "").trim();
    if (API_ERROR_CODES.has(cleanMessage))
        return API_ERROR_CODES.get(cleanMessage);
    if (cleanMessage.startsWith("Debe esperar"))
        return "rate_limit_exceeded";
    if (cleanMessage.startsWith("Has alcanzado el límite de consultas diarias"))
    ...
    if (status === 409)
        return "concurrent_idempotent_request";
    if (status === 429)
        return "rate_limit_exceeded";
    if (status === 503)
...
function formatClientError(curp, error) {
    return `❌ Sobre tu consulta de la CURP ${curp}:\n\n${error.message}`;
}
function normalizeError(error, uuid = "") {
    if (error instanceof ScPremiumError) {
        if (!error.uuid && uuid)
            error.uuid = uuid;
        return error;
    }
    return new ScPremiumError({ message: error?.message || String(error), stage: "unexpected", code: "unexpected", uuid, cause: error });
}
function extractUuid(data) {
    return String(data?.uuid || "").trim();
}
function extractNss(submitData, completedData) {
    const value = findFirstValue([submitData, completedData], ["nss", "result.nss", "data.nss"]);
    return String(value || "").replace(/\D/g, "").trim();
}
function extractFullName(submitData, completedData) {
...
function findFirstValue(sources, paths) {
...
async function sleepBeforeDeadline(milliseconds, deadline) {
...
async function react(mctx, emoji) {
    try {
...
function logInternalError(error) {
    if (String(process.env.SC_PREMIUM_DEBUG || "") !== "1")
        return;
