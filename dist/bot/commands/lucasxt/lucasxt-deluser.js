...
import { getTargetJids, refreshGroupMetadata, resolveGroupActionTarget, sameIdentity, } from "../../../libs/lucasxt-moderation.js";
...
export default {
    ...
    execute: async (wss, { mctx, args, bot, groupMetadata }) => {
        ...
        if (!rawTargets.length) {
            await mctx.reply(card("deluser", [`Menciona, responde o pasa el número del usuario a eliminar.`]));
            return;
        }
        let identities = [];
        if (mctx.is_group) {
        ...
        if (identities.some((jid) => getBotOwnerIdentityJids(bot).some((ownerJid) => sameIdentity(jid, ownerJid)) ||
        ...
        await mctx.reply(card("Usuario eliminado", [
