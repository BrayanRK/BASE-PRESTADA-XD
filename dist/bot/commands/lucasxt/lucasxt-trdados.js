...
import { DICE_SLOTS, addDiceEntry, getRandomDiceEntry, listDiceEntries, nextEmptySlot, readDiceBuffer, } from "../../../libs/dice-stickers.js";
const card = (title, lines = []) => [`「◈」 *${title}*`, ...lines].join("\n");
...
const sendDiceEntry = async (wss, mctx, entry, buffer) => {
    if (entry.kind === "sticker") {
        await wss.sendMessage(mctx.chat.jid, { sticker: buffer }, { quoted: mctx.message.original });
        return;
    }
    if (entry.kind === "video") {
        await wss.sendMessage(mctx.chat.jid, { video: buffer, gifPlayback: true, mimetype: entry.mimetype || "video/mp4" }, { quoted: mctx.message.original });
        return;
    }
...
const stAddCommand = {
    ...
    execute: async (_wss, { mctx, args }) => {
        ...
        if (!slot) {
            await mctx.reply(card("stadd", [`Los ${DICE_SLOTS} slots están llenos.`, `Usa *.stadd <slot 1-${DICE_SLOTS}>* para reemplazar uno.`]));
            return;
        }
        let buffer;
        try {
