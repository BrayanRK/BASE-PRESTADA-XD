...
import { dvyerYtSearch, dvyerTitle, dvyerAuthor, dvyerDuration, dvyerLink, dvyerThumb, dvyerUserError, evogbSearchYt, evogbTitle, evogbAuthor, evogbDuration, evogbLink, evogbThumb } from "../../../libs/downloads.js";
const short = (v, max = 1800) => v.length > max ? `${v.slice(0, max - 3)}...` : v;
const clean = (v) => String(v ?? "").replace(/\s+/g, " ").trim();
const views = (v) => { const n = Number(clean(v).replace(/[^\d]/g, "")) || 0; return n ? formatViews(n) : clean(v) || "N/A"; };
const buildText = (items, getTitle, getAuthor, getDuration, getLink) => items.slice(0, 5).map((item, i) => [
    `${i + 1}. ${getTitle(item)}`,
    `Canal: ${getAuthor(item)}`,
    getDuration(item) ? `Duración: ${getDuration(item)}` : "",
    `Vistas: ${views(item.views || item.viewCount)}`,
...
export default {
    ...
    execute: async (wss, { mctx, args }) => {
        ...
        try {
            ...
            await mctx.react("❌");
            await mctx.reply(`「✖」 ${dvyerUserError(e, "No se pudo realizar la búsqueda.")}`);
        }
    },
};
