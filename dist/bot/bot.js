...
export class Bot {
    ...
    connect = async (reconnecting = false) => {
        const sessionPath = this.config.session_path || this.getDefaultSessionPath();
        if (isSocketStopped(this.config.bot_jid || path.basename(sessionPath)))
            return;
        try {
    ...
    extendSocket = (sock) => {
        ...
        if (typeof sock.requestPairingCode === "function") {
            wss.requestPairingCode = sock.requestPairingCode.bind(sock);
        }
        if (typeof sock.onWhatsApp === "function") {
            wss.onWhatsApp = sock.onWhatsApp.bind(sock);
        }
        wss.groupMetadata = async (jid, incache = true) => {
            if (!/@g\.us$/.test(jid))
                return null;
        ...
        wss.profilePictureUrl = async (jid, type = "image") => {
            try {
                return await realProfilePictureUrl(jid, type);
            }
            catch {
    ...
    getPremiumBackupPath(sessionPath) {
        const projectRoot = path.resolve(process.cwd());
    ...
    async restorePremiumBackup(sessionPath) {
    ...
    setupEventHandlers = (wss, saveCreds, sessionPath) => {
        wss.ev.on("creds.update", async () => {
            await saveCreds();
            await this.backupPremiumSession(sessionPath);
        });
        wss.ev.on("connection.update", (e) => this.handleConnectionUpdate(wss, e));
    ...
    handleConnectionUpdate = async (wss, e) => {
        ...
        if ("connection" in e && e.connection === "open")
            await this.handleOpen(wss);
        if ("connection" in e && e.connection === "close")
            await this.handleClose(wss, e.lastDisconnect);
    };
    schedulePairingCode(wss, delayMs = 0) {
        if (this.config.connection_method !== "code" || !this.config.bot_jid)
            return;
        if (this.codesSent >= 1 || this.pairingCodeRequested)
            return;
    ...
    async requestCodeWithTimeout(wss, phoneNumber, timeoutMs = 10_000) {
        const requester = wss.requestPairingCode;
        if (typeof requester !== "function") {
            throw new Error("El socket no tiene requestPairingCode disponible");
        }
        return Promise.race([
            requester(phoneNumber),
            new Promise((_, reject) => setTimeout(() => reject(new Error("WhatsApp no entregó el código a tiempo")), timeoutMs)),
        ]);
    }
    async emitPairingCode(wss) {
        if (!this.config.bot_jid) {
            this.ev.emit("bot.error", { error: "Número no configurado" });
            return;
        }
        if (this.pairingCodeRequested || this.codesSent >= 1)
            return;
        ...
        try {
            ...
            for (let attempt = 1; attempt <= 8; attempt++) {
                if (this.codesSent >= 1)
                    return;
                if (wss.authState?.creds?.registered)
                    return;
                ...
                try {
                    ...
                    if (!code)
                        throw new Error("WhatsApp devolvió un código vacío");
                    this.codesSent++;
    ...
    async handleOpen(wss) {
        const botJid = baileys.jidNormalizedUser(wss.user?.id || "");
        if (!botJid)
            return;
        ...
        const botLid = (() => {
            try {
                return baileys.jidNormalizedUser(wss.user?.lid || "");
            }
            catch {
                return "";
            }
        ...
        const effectiveOwnerJid = effectiveBotType === "premium"
        ...
        const effectiveOwnerName = savedBot?.owner_name ||
            (effectiveBotType === "premium"
                ? ""
                : effectiveOwnerJid
                    ? await wss.getName(effectiveOwnerJid).catch(() => getRuntimeOwnerName())
                    : getRuntimeOwnerName());
        const effectiveParentBotJid = effectiveBotType === "free" ? savedBot?.parent_bot_jid || this.config.parent_bot_jid || "" : "";
    ...
    async handleClose(wss, lastDisconnect) {
        ...
        const awaitingPairingCodeLink = this.config.connection_method === "code" &&
            !this.openedOnce &&
            this.codesSent >= 1;
    ...
    delete = async () => {
        ...
        try {
            ...
            if (botNumber && this.config.bot_type !== "premium")
                db.run("UPDATE premium_codes SET is_active = 0 WHERE bot_number = ?", [botNumber], () => { });
            if (botJid) {
