...
export class Users {
    ...
    static set = async (key, value) => {
        return new Promise(async (resolve) => {
            try {
                ...
                db.run(`INSERT INTO users (user_jid, name, range, level, experience) VALUES (?, ?, ?, ?, ?)`, [userData.user_jid, userData.name, userData.range, userData.level, userData.experience], async (err) => {
                    if (err) {
    ...
    static update = async (key, update) => {
        return new Promise(async (resolve) => {
            try {
    ...
    static has = async (key) => {
        return new Promise((resolve) => {
            try {
                const db = getConnection();
                db.get("SELECT COUNT(*) as count FROM users WHERE user_jid = ?", [key], (err, row) => {
                    if (err) {
...
export class Groups {
    static get = async (key) => {
        return new Promise((resolve) => {
            try {
                const db = getConnection();
                db.get("SELECT * FROM groups_table WHERE group_jid = ?", [key], (err, group) => {
                    if (err) {
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                const groupData = {
                    ...
                    antidelete_enabled: value.antidelete_enabled ? 1 : 0,
                    welcomes_enabled: value.welcomes_enabled ? 1 : 0,
                    welcome_message: value.welcome_message || "",
                    welcome_image_url: value.welcome_image_url || "",
                    farewells_enabled: value.farewells_enabled ? 1 : 0,
                    farewell_message: value.farewell_message || "",
                    farewell_image_url: value.farewell_image_url || "",
                    alerts_enabled: value.alerts_enabled ? 1 : 0,
                };
                db.run(`INSERT INTO groups_table (group_jid, admins_only_enabled, primary_bot, moderators_only_enabled,
                        ...
                        db.run(`UPDATE groups_table SET ${updates.join(", ")} WHERE group_jid = ?`, values, async (err) => {
                            if (err) {
                                ...
                                if (update.$push && update.$push.users) {
                                    ...
                                    db.run(`INSERT OR IGNORE INTO group_users (group_jid, user_jid, money, money_deposited, berries, enhancers,
    ...
    static has = async (key) => {
        return new Promise((resolve) => {
            try {
                const db = getConnection();
                db.get("SELECT COUNT(*) as count FROM groups_table WHERE group_jid = ?", [key], (err, row) => {
                    if (err) {
    ...
    static size = async () => {
        return new Promise((resolve) => {
            try {
                const db = getConnection();
                db.get("SELECT COUNT(*) as count FROM groups_table", [], (err, row) => {
                    if (err) {
...
const normalizeSqlBool = (value) => {
    return value === true || value === 1 || value === "1" ? 1 : 0;
};
const pickText = (incoming, current, fallback = "") => {
...
export class Bots {
    ...
    static set = async (key, value) => {
        return new Promise(async (resolve) => {
            try {
                ...
                const effectiveBotType = (value.bot_type || current?.bot_type || "main");
                ...
                const botData = {
                    ...
                    setup_completed: value.setup_completed !== undefined && value.setup_completed !== null
    ...
    static listByType = async (types_) => {
        return new Promise((resolve) => {
            try {
                if (!types_.length) {
                    resolve([]);
                ...
                db.all(`SELECT * FROM bots WHERE bot_type IN (${placeholders})`, types_, (err, rows) => {
                    if (err) {
                        ...
                        resolve((rows || []).map(hydrateBotDocument));
                    }
                });
            }
            catch (error) {
                        ...
                        db.run(`UPDATE bots SET ${updates.join(", ")} WHERE bot_jid = ?`, values, async (err) => {
                            if (err) {
                                ...
                                if (ownerJid || parentBotJid) {
                                    ...
                                    db.run(`UPDATE bot_sessions SET ${sessionUpdates.join(", ")} WHERE bot_jid = ? OR bot_number = ?`, sessionValues, () => { });
                                }
                                resolve(await this.get(key));
                            }
                        });
    ...
    static has = async (key) => {
        return new Promise((resolve) => {
            try {
                const db = getConnection();
                db.get("SELECT COUNT(*) as count FROM bots WHERE bot_jid = ?", [key], (err, row) => {
                    if (err) {
...
const cleanDbText = (value) => String(value ?? "").trim();
const dbJidNumber = (jid) => cleanDbText(jid).split(":")[0].split("@")[0].replace(/[^0-9]/g, "");
const normalizeDbJid = (jid) => {
...
const normalizeBotKey = (botJid) => normalizeDbJid(botJid) || cleanDbText(botJid);
const dbJidServer = (jid) => normalizeDbJid(jid).split("@")[1] || "";
const sameDbUserIdentity = (left, right) => {
    ...
    if (a === b)
        return true;
    if (dbJidServer(a) === "lid" || dbJidServer(b) === "lid")
        return false;
...
export class BotSubOwners {
    static add = async (botJid, userJid, addedBy = "") => {
        return new Promise((resolve) => {
            try {
                ...
                const db = getConnection();
                db.run(`INSERT INTO bot_subowners (bot_jid, user_jid, added_by)
           VALUES (?, ?, ?)
           ON CONFLICT(bot_jid, user_jid) DO UPDATE SET added_by = excluded.added_by`, [botKey, userKey, addedByKey], (err) => {
                    if (err) {
    ...
    static remove = async (botJid, userJid) => {
        return new Promise((resolve) => {
            try {
                const botKey = normalizeBotKey(botJid);
                const userKey = normalizeDbJid(userJid);
    ...
    static has = async (botJid, userJid) => {
        return new Promise((resolve) => {
            try {
                const botKey = normalizeBotKey(botJid);
                const userKey = normalizeDbJid(userJid);
                if (!botKey || !userKey)
                    return resolve(false);
                const db = getConnection();
                db.all("SELECT user_jid FROM bot_subowners WHERE bot_jid = ?", [botKey], (err, rows) => {
                    if (err) {
...
export class BotSettings {
    static get = async (botJid, key) => {
        return new Promise((resolve) => {
            try {
                const botKey = normalizeBotKey(botJid);
                const settingKey = cleanDbText(key);
                if (!botKey || !settingKey)
                    return resolve("");
                const db = getConnection();
                db.get("SELECT value FROM bot_settings WHERE bot_jid = ? AND key = ?", [botKey, settingKey], (err, row) => {
                    if (err) {
    ...
    static set = async (botJid, key, value) => {
        return new Promise((resolve) => {
            try {
                const botKey = normalizeBotKey(botJid);
                const settingKey = cleanDbText(key);
                if (!botKey || !settingKey)
                    return resolve(false);
                const db = getConnection();
                db.run(`INSERT INTO bot_settings (bot_jid, key, value)
           VALUES (?, ?, ?)
           ON CONFLICT(bot_jid, key) DO UPDATE SET value = excluded.value, updated_at = CURRENT_TIMESTAMP`, [botKey, settingKey, cleanDbText(value)], (err) => {
                    if (err) {
    ...
    static getBool = async (botJid, key, fallback = false) => {
        const value = await this.get(botJid, key);
        if (!value)
            return fallback;
        return /^(1|true|yes|on|si|sí)$/i.test(value);
    };
    static setBool = async (botJid, key, value) => {
