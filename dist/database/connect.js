...
let db = null;
let opening = null;
const dbPath = () => path.join(process.cwd(), "database", "sqlite", "zeta.sqlite");
...
const runAsync = (conn, sql, params = []) => new Promise((resolve, reject) => {
    conn.run(sql, params, (err) => (err ? reject(err) : resolve()));
});
const execAsync = (conn, sql) => new Promise((resolve, reject) => {
    conn.exec(sql, (err) => (err ? reject(err) : resolve()));
});
const applyPragmas = async (conn) => {
