...
import { getRuntimeAssetPath, getUniversalConfig, normalizeOwnerJid, normalizeOwnerNumber, readUniversalConfig, updateUniversalConfig, writeUniversalConfig, } from "./libs/zeta_cf.js";
const { useMultiFileAuthState, DisconnectReason, Browsers } = baileys;
const originalConsoleError = console.error;
console.error = (...args) => {
    const message = args.map((arg) => (typeof arg === "string" ? arg : String(arg?.message || arg))).join(" ");
    const noisyBaileysErrors = [
        "No SenderKeyRecord found for decryption",
        "Bad MAC",
        "Session error",
    ];
    if (noisyBaileysErrors.some((text) => message.includes(text)))
        return;
...
const printStep = (step, title, description) => {
    console.log("");
...
const askRequiredStep = async (step, title, description) => {
...
const askOwnerNumberStep = async () => {
...
const ensureInitialConfig = async () => {
    ...
    const botName = await askRequiredStep(1, "Nombre destinado al bot", "Escribe el nombre final que usará este bot. No puede quedar vacío.");
    const ownerNumber = await askOwnerNumberStep();
    const currencyName = await askRequiredStep(3, "Nombre de la moneda de economía", "Ejemplo: SkyCoins, ZetaCoins, Diamantes. No puede quedar vacío.");
    const ownerName = await askRequiredStep(4, "Nombre del owner/dueño del bot", "Escribe el nombre público del dueño que usará la base.");
...
const cleanStoredText = (value) => String(value || "").trim();
const pickStoredBot = (bots, config) => {
    const existing = bots.filter(Boolean);
    if (!existing.length)
        return null;
    return (existing.find((bot) => cleanStoredText(bot.owner_jid)) ||
...
const syncRuntimeBotRecord = async () => {
    ...
    jids.add(safeNormalizeJid(conn.user?.lid));
...
const extendSocket = (sock) => ({
    ...
    groupMetadata: async (jid, useCache = true) => {
        ...
        try {
            ...
            if (metadata)
                cache.metadatas.set(jid, metadata);
            return metadata;
        }
        catch {
            return null;
...
const startBot = async (authState, saveCredentials) => {
    try {
...
const formatConnectionError = (error) => {
    ...
    const message = payloadMessage || error?.message || String(error || "error desconocido");
    return statusCode ? `${message} (${statusCode})` : message;
};
const isRestartRequired = (error) => {
...
const savePendingPairingState = async (phoneNumber) => {
...
const readPendingPairingState = () => {
    try {
        if (!fs.existsSync(pairingStateFile))
            return null;
        const raw = fs.readFileSync(pairingStateFile, "utf8");
        const parsed = JSON.parse(raw);
        if (!parsed?.phoneNumber || typeof parsed.createdAt !== "number")
            return null;
...
const schedulePairingTimeout = () => {
    ...
    pairingTimeout = setTimeout(() => {
        if (!pendingPairingPhoneNumber || isConnected || state?.creds?.registered)
            return;
...
const isCleanPairingClose = (error) => {
    const statusCode = getErrorStatusCode(error);
...
const finishWithPairingFailure = async (error) => {
...
const requestRuntimePairingCode = async () => {
    if (!pendingPairingPhoneNumber || pairingCodeRequested || isConnected)
        return;
    if (state?.creds?.registered)
        return;
    pairingCodeRequested = true;
    try {
        const code = await conn.requestPairingCode(pendingPairingPhoneNumber);
...
const showConnectionMenu = async () => {
...
const cleanPairingPhoneNumber = (phoneNumber) => {
...
const getAuthCredsPath = () => path.resolve(process.cwd(), authFile, "creds.json");
const hasSavedValidSession = (authState) => {
    if (!fs.existsSync(getAuthCredsPath()))
        return false;
    const creds = authState?.creds;
    return Boolean(creds?.registered === true || creds?.me?.id);
};
const startWithSavedSession = async (authState, saveCredentials) => {
    resetPairingRuntime();
    await clearPendingPairingState();
    useQRMode = false;
    closeConsoleInput();
...
const archiveAuthState = async () => {
    ...
    if (!fs.existsSync(authPath))
        return;
    const archivePath = path.resolve(process.cwd(), `${authFile}-invalid-${new Date().toISOString().replace(/[:.]/g, "-")}`);
...
const handlePairingCode = async (phoneNumber, autoResume = false) => {
...
const main = async () => {
    ...
    try {
        ...
        if (connectionChoice === "2") {
            const phoneNumber = await askRequiredStep(7, "Número para código de emparejamiento", "Escribe el número con código de país, solo dígitos si quieres. Ejemplo: 51999999999");
            console.log(chalk.yellow("🔑 Generando código de emparejamiento..."));
...
process.on("uncaughtException", (err) => {
    if (pendingPairingPhoneNumber && isCleanPairingClose(err))
        return;
    console.log(chalk.red(`❌ Error no capturado: ${formatConnectionError(err)}`));
});
process.on("unhandledRejection", (reason) => {
    if (pendingPairingPhoneNumber && isCleanPairingClose(reason))
        return;
